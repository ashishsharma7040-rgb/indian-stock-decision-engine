...
def live_debug_payload() -> dict[str, Any]:
    ...
    pending_symbols = status.get("pending_subscription_symbols") or []
    login_success_at = status.get("last_login_success_at")
    credential_diagnostics = status.get("credential_diagnostics") or {}
    runtime_twofa_age_seconds = status.get("runtime_twofa_age_seconds")
    attempted_password_modes = status.get("attempted_password_modes") or []
    likely_blockers: list[str] = []
    credential_assessment: list[dict[str, Any]] = []
    login_timeout_seconds = max(10, int(os.getenv("SHOONYA_LOGIN_TIMEOUT_SECONDS", "35")))
    websocket_timeout_seconds = max(10, int(os.getenv("SHOONYA_WEBSOCKET_START_TIMEOUT_SECONDS", "20")))
    attempt_age_seconds: int | None = None
    attempted_at_raw = status.get("last_login_attempt_at")
    if attempted_at_raw:
        try:
            attempt_age_seconds = max(
                0,
                int(
                    (
                        datetime.now(timezone.utc)
                        - datetime.fromisoformat(str(attempted_at_raw).replace("Z", "+00:00"))
                    ).total_seconds()
                ),
            )
        except Exception:
    ...
    if not configured:
        ...
        next_step = "Install or verify the Shoonya SDK package on Render, switch to PYTHON_VERSION=3.11.9, then redeploy."
    elif status.get("status") == "login_attempting" and attempt_age_seconds is not None and attempt_age_seconds > (login_timeout_seconds + 5):
        next_step = "Shoonya login is stuck beyond the expected timeout. This points to the broker SDK hanging, Render outbound connectivity trouble, or the broker not accepting the OTP/TOTP."
    elif status.get("runtime_twofa_set") and status.get("totp_source") != "runtime_twofa":
        next_step = "Runtime OTP is stored for the next login attempt. Click Start / retry live feed or wait for reconnect."
    elif "timed out after" in str(last_error):
        next_step = "Shoonya login or websocket startup timed out. This points to broker response delay, Render outbound connectivity, or the SDK hanging before the socket opens."
    elif "Invalid SHOONYA_TOTP_SECRET format" in str(last_error):
        next_step = "The TOTP secret format is invalid. Re-copy the base32 Shoonya TOTP secret into Render without spaces or quotes."
    elif status.get("status") == "login_attempting":
        next_step = "Login is in progress. If this state persists, check Render outbound connectivity and Shoonya OTP/TOTP acceptance."
    elif status.get("last_login_response_stat") == "Ok" and status.get("websocket_open_attempted") and not feed_open:
        next_step = "Shoonya login succeeded, but the WebSocket did not open. This points to a websocket startup, broker-side, or Render network issue."
    elif status.get("status") == "logged_in" and not feed_open:
        next_step = "Shoonya login succeeded, but the WebSocket is not open yet. This points to a websocket startup or network issue."
    elif last_error:
        next_step = "Review the exact login error, then verify host endpoints, API key/vendor code pairing, and OTP freshness."
    elif pending_symbols and not feed_open:
        next_step = "Login is still warming up. Pending symbols will subscribe automatically once the WebSocket opens."
    elif pending_symbols and feed_open:
        next_step = "WebSocket is open. Pending symbols are being resolved and subscribed in the background."
    elif not feed_open:
    ...
    if pending_symbols:
        next_step = f"{next_step} Pending symbols: {', '.join(pending_symbols[:8])}."

    if credential_diagnostics.get("vendor_code", {}).get("looks_placeholder"):
        likely_blockers.append("Vendor code looks like a placeholder. Use the real Shoonya vendor code from your broker setup.")
    if credential_diagnostics.get("api_key", {}).get("looks_placeholder") or int(credential_diagnostics.get("api_key", {}).get("length") or 0) < 8:
        likely_blockers.append("API key looks incomplete. SHOONYA_API_KEY should be the real Prism generated API key, not a placeholder.")
    if credential_diagnostics.get("imei", {}).get("looks_placeholder") or int(credential_diagnostics.get("imei", {}).get("length") or 0) < 6:
        likely_blockers.append("IMEI/device id looks suspiciously short. Use the exact device id / imei value expected by your Shoonya setup.")
    if credential_diagnostics.get("vendor_code", {}).get("present") and "empty/non-json login response" in str(last_error).lower():
        likely_blockers.append("Vendor code should come from Prism along with the API key. It is not the same thing as your user id or password.")
    if status.get("totp_source") == "runtime_twofa" and runtime_twofa_age_seconds is not None and runtime_twofa_age_seconds > 45:
        likely_blockers.append("Manual OTP is older than 45 seconds and may be stale. Generate a fresh OTP and retry immediately.")
    if "empty/non-JSON login response" in str(last_error):
        likely_blockers.append("Shoonya is not returning a normal login JSON response. Most likely causes are API key/vendor code mismatch, stale OTP, or broker-side rejection from Render.")
    if set(attempted_password_modes) == {"raw"}:
        likely_blockers.append("Only raw password mode was tried. If your broker setup expects hashed password login, enable auto mode or redeploy the latest backend so it tries both raw and SHA256.")
    if set(attempted_password_modes) == {"raw", "sha256"}:
        likely_blockers.append("Both raw and SHA256 password modes were tried. If login still fails, the remaining likely issue is API key/vendor code/OTP correctness rather than password formatting.")
    if credential_diagnostics.get("totp_secret", {}).get("present") and not credential_diagnostics.get("totp_secret", {}).get("format_valid"):
        likely_blockers.append("Configured TOTP secret format looks invalid. Re-copy the base32 secret without spaces.")
    if not likely_blockers:
        likely_blockers.append("No obvious configuration formatting problem was detected. The next suspects are broker-side credential rejection or network restrictions.")

    user_id_len = int(credential_diagnostics.get("user_id", {}).get("length") or 0)
    vendor_len = int(credential_diagnostics.get("vendor_code", {}).get("length") or 0)
    api_key_len = int(credential_diagnostics.get("api_key", {}).get("length") or 0)
    imei_len = int(credential_diagnostics.get("imei", {}).get("length") or 0)
    runtime_name = os.getenv("PYTHON_VERSION", "").strip() or platform.python_version()

    credential_assessment.append(
        {
            "label": "Python runtime",
            "status": "warn" if runtime_name.startswith("3.13") else "ok",
            "detail": (
                f"Current runtime looks like {runtime_name}. Shoonya SDK is safer on Python 3.11.9."
                if runtime_name.startswith("3.13")
                else f"Current runtime looks like {runtime_name}."
            ),
        }
    )
    credential_assessment.append(
        {
            "label": "Vendor code pattern",
            "status": "ok" if vendor_len > user_id_len and str(status.get("credential_flags", {}).get("vendor_code")).lower() != "false" else "warn",
            "detail": "Shoonya FAQ says vendor code is usually client-code suffixed with _U. Yours looks structurally plausible." if vendor_len > user_id_len else "Vendor code does not obviously look like the documented client-code_U pattern.",
        }
    )
    credential_assessment.append(
        {
            "label": "IMEI / device id",
            "status": "ok" if imei_len >= 4 else "warn",
            "detail": "Shoonya FAQ allows any chosen device identifier such as abc1234, so the current IMEI value shape is probably acceptable." if imei_len >= 4 else "IMEI / device id looks too short.",
        }
    )
    credential_assessment.append(
        {
            "label": "API key",
            "status": "warn",
            "detail": "This cannot be verified by format alone. If login still fails after trying both password modes, the API key is the top credential suspect. It must be the Prism generated API key.",
        }
    )
    credential_assessment.append(
        {
            "label": "OTP freshness",
            "status": "warn" if runtime_twofa_age_seconds is not None and runtime_twofa_age_seconds > 30 else "ok",
            "detail": (
                f"Runtime OTP is already {runtime_twofa_age_seconds}s old. Generate OTP and submit/start immediately."
                if runtime_twofa_age_seconds is not None and runtime_twofa_age_seconds > 30
                else "Submit OTP and start login immediately while it is still fresh."
            ),
        }
    )
    credential_assessment.append(
        {
            "label": "Password mode coverage",
            "status": "ok" if set(attempted_password_modes) == {"raw", "sha256"} else "warn",
            "detail": (
                f"Attempted modes: {', '.join(attempted_password_modes)}."
                if attempted_password_modes
                else "No password modes attempted yet."
            ),
        }
    )
    ...
    return {
        ...
        "credential_flags": status.get("credential_flags", {}),
        ...
        "last_login_attempt_at": status.get("last_login_attempt_at"),
        "login_attempt_age_seconds": attempt_age_seconds,
        "last_login_success_at": login_success_at,
        "last_login_response_stat": status.get("last_login_response_stat"),
        "last_login_error": status.get("last_login_error"),
        "websocket_open_attempted": bool(status.get("websocket_open_attempted")),
        "websocket_opened_at": status.get("websocket_opened_at"),
        "websocket_last_close_reason": status.get("websocket_last_close_reason"),
        "last_endpoint_host": status.get("last_endpoint_host"),
        "last_endpoint_websocket": status.get("last_endpoint_websocket"),
        "totp_source": status.get("totp_source"),
        ...
        "unresolved_symbols": status.get("unresolved_symbols", []),
        "pending_subscription_symbols": pending_symbols,
        "pending_subscription_count": status.get("pending_subscription_count", 0),
        "runtime_twofa_set_at": status.get("runtime_twofa_set_at"),
        "runtime_twofa_last_used_at": status.get("runtime_twofa_last_used_at"),
        "runtime_twofa_age_seconds": runtime_twofa_age_seconds,
        "credential_diagnostics": credential_diagnostics,
        "credential_assessment": credential_assessment,
        "attempted_password_modes": attempted_password_modes,
        "last_login_password_mode": status.get("last_login_password_mode"),
...
async def startup_refresh_and_subscribe() -> None:
    ...
    provider = str(os.getenv("LIVE_DATA_PROVIDER", "")).strip().lower()
    in_app_feed = str(os.getenv("SHOONYA_IN_APP_FEED", "true")).lower() in {"1", "true", "yes"}
    market_data_only = str(os.getenv("BROKER_MODE", "market_data_only")).strip().lower() == "market_data_only"
    trading_enabled = str(os.getenv("ENABLE_TRADING", "false")).strip().lower() == "true"
    if provider == "shoonya" and live_feed.configured and in_app_feed and market_data_only and not trading_enabled:
        try:
            await asyncio.to_thread(live_feed.start)
            await asyncio.sleep(8)
            safe_symbols = WATCH_SYMBOLS[: min(len(WATCH_SYMBOLS), 25)]
            await asyncio.to_thread(live_feed.subscribe, safe_symbols)
            print(f"[startup] Shoonya start attempted. Safe subscribe for {len(safe_symbols)} symbols.")
        except Exception as exc:  # pragma: no cover - broker/network dependent
            print(f"[startup] Shoonya startup failed: {exc}")
    elif provider == "shoonya" and live_feed.configured:
...
def stock_summary(stock: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "research_summary": stock.get("research_summary"),
        "short_term_view": stock.get("short_term_view"),
        "long_term_view": stock.get("long_term_view"),
        "pro_research_sections": stock.get("pro_research_sections"),
        "risk_dashboard": stock.get("risk_dashboard"),
        "data_quality_explanation": stock.get("data_quality_explanation"),
...
def build_scan_wait_data_result(
    ...
    *,
    ...
    return {
        ...
        "research_summary": {
            "headline": "WAIT_DATA",
            "time_horizon": "Research only",
            "setup_state": "Screened",
            "why_interesting": f"{symbol} is in the research universe, but the engine is still waiting for enough valid data.",
            "why_not_actionable": reason,
            "main_trigger": "No valid trigger until data quality improves.",
            "main_risk": reason,
            "orientation": "short_term",
        },
        "short_term_view": {
            "score": 0,
            "technical_score": 0,
            ...
            "relative_strength_vs_nifty": {"state": "Unavailable", "pct": None},
            "trigger": None,
            "pullback_zone": None,
            "stop": None,
            "target_1": None,
        ...
        "long_term_view": {
            ...
            "metrics": {},
            "business_quality_explanation": "Business-quality analysis is blocked until enough data is available.",
            "sector_tailwind_explanation": "Sector-tailwind analysis is limited while the data gate is blocked.",
            "watchlist_only": True,
        },
        "pro_research_sections": {
            "bull_case": ["The stock can stay on the research queue while data coverage improves."],
            "bear_case": [reason],
            "neutral_case": ["Do not rank/action until data-quality gate passes."],
            "checklist_before_action": [
                "Refresh the stock after database history and fundamentals improve.",
                "Confirm the data-quality gate before acting on any trigger.",
            ],
        },
        "risk_dashboard": {
            "business_risk": "Unknown",
            "valuation_risk": "Unknown",
            "technical_risk": "History unavailable",
            "data_risk": reason,
        ...
        "data_quality_explanation": {
            ...
            "bars_available": bars_available,
            "corporate_actions_applied": False,
            "recent_source": "Supabase official latest bhavcopy",
            "latest_bar_date": latest_bar_date,
            "real_vs_fallback": "Blocked",
...
def live_status(start: int = 0) -> dict[str, Any]:
    if start and live_feed.configured:
        live_feed.start()
    return live_feed.status()


@app.get("/api/live/debug")
def live_debug(start: int = 0) -> dict[str, Any]:
    if start and live_feed.configured:
        live_feed.start()
...
def live_twofa(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        live_feed.set_runtime_twofa(twofa)
        response = live_debug_payload()
        response["otp_submission"] = {
            "accepted": True,
            "mode": "runtime_override",
            "message": "OTP accepted by backend. It is stored as a one-time override for the next login attempt. Click Start / retry live feed now.",
        }
        return response
