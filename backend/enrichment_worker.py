...
async def enrich_symbol(symbol: str) -> dict[str, Any]:
    ...
    bars = await asyncio.to_thread(fetch_yahoo_chart, symbol, "2y", "1d")
    fundamentals = await asyncio.to_thread(fetch_yahoo_fundamentals, symbol)
    async with AsyncSessionLocal() as session:
...
async def run_enrichment_pipeline(
    ...
    counts = await _existing_adjusted_history_counts(requested_symbols)
    symbols = requested_symbols if force else [symbol for symbol in requested_symbols if counts.get(symbol, 0) < min_existing_bars]
    total = len(symbols)
    ...
    status: dict[str, Any] = {
        ...
        "skipped_existing": len(requested_symbols) - total,
        "processed": 0,
        "enriched": 0,
        "history_rows": 0,
        "fundamentals": 0,
    ...
    publish()
    if total == 0:
        status.update({"status": "complete", "progress": 100, "message": "All selected symbols already have sufficient Yahoo history."})
        publish()
        return status
    ...
    async def worker(worker_id: int) -> None:
        ...
        while True:
            ...
            try:
                ...
                async with lock:
                    status["processed"] += 1
                    if int(row.get("history_rows") or 0) > 0:
                        status["enriched"] += 1
