...
def build_full_nse_scan_payload(scan_limit: Any = "all", force: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        # Guard against stale seed-only cache. Real scan rows must have EOD price/as_of
        # or turnover/volume from a refreshed bhavcopy.
        rows_to_score = [
            row
            for row in rows_to_score
            if row.get("as_of") or row.get("turnover") or row.get("volume")
        ]
...
def run_full_nse_scan(
    ...
    if FULL_NSE_SCAN_STATUS.get("status") in {"queued", "running"}:
        return {
