...
class PriceHistoryDaily(Base):
    ...
    volume = Column(Integer)
    turnover = Column(Float)
    data_source = Column(String, default="unknown", nullable=False)
    is_adjusted = Column(Boolean, default=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())

    __table_args__ = (UniqueConstraint("symbol", "date", "data_source", name="uix_price_history_symbol_date_source"),)
...
class Nifty500Constituent(Base):
    __tablename__ = "nifty500_constituents"
...
class MarketBreadthHistory(Base):
    __tablename__ = "market_breadth_history"

    date = Column(Date, primary_key=True)
    nifty500_breadth_pct = Column(Float)
    stocks_above_50dma = Column(Integer)
...
class ScanRun(Base):
    ...
    ranking_eligible_count = Column(Integer, default=0)
    insufficient_history_count = Column(Integer, default=0)
    data_quality_failed_count = Column(Integer, default=0)
    error = Column(String)
...
class ScanResult(Base):
    ...
    id = Column(Integer, primary_key=True)
    scan_run_id = Column(Integer, ForeignKey("scan_runs.id", ondelete="CASCADE"), index=True, nullable=False)
    symbol = Column(String, ForeignKey("companies.symbol", ondelete="CASCADE"), index=True, nullable=False)
    rank_weekly = Column(Integer)
    rank_monthly = Column(Integer)
    weekly_score = Column(Integer)
    monthly_score = Column(Integer)
    conviction = Column(String)
    ranking_eligible = Column(Boolean, default=False, index=True)
    data_mode = Column(String)
    data_quality_gate = Column(JSON, default=dict)
    action_plan = Column(JSON, default=dict)
    raw_payload = Column(JSON, default=dict)
    created_at = Column(DateTime(timezone=True), server_default=func.now())

    __table_args__ = (UniqueConstraint("scan_run_id", "symbol", name="uix_scan_result_run_symbol"),)


Index("idx_daily_ohlcv_symbol_date", DailyOHLCV.symbol, DailyOHLCV.date.desc())
Index("idx_price_history_symbol_date", PriceHistoryDaily.symbol, PriceHistoryDaily.date.desc())
...
Index("idx_scan_runs_status_finished", ScanRun.status, ScanRun.finished_at.desc())
Index("idx_scan_results_run_rank", ScanResult.scan_run_id, ScanResult.rank_weekly, ScanResult.rank_monthly)
