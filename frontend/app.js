...
function normalizeFullScanPayload(payload) {
  ...
  const filtered = stocks.filter(stock => {
    ...
    const realMode = !isFullScan || ["real_nse_scored", "nse_dynamic_real", "nse_dynamic_focus"].includes(stock.data_mode || "");
...
async function runFullNseScan() {
  ...
  const applyFullScanPayload = (payload) => {
    ...
    lastSuccessfulFetch = Date.now();
    currentDashboardDelay = DASHBOARD_POLL_MS;
    setScanUi(`Scan complete. Universe ${scanMeta.universe_size || "?"}, passed ${scanMeta.passed_liquidity || "?"}, real scored ${scanMeta.total_scored || "?"}, ranked ${scanMeta.ranked_candidates || "?"}.`);
  };
