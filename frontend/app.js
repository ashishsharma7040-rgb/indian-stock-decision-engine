...
function normalizeFullScanPayload(payload) {
  ...
  const filtered = stocks.filter(stock => {
    ...
    const realMode = !isFullScan || ["real_nse_scored", "nse_eod_proxy_scored", "nse_dynamic_real", "nse_dynamic_focus"].includes(stock.data_mode || "");
    return realMode && weekly >= 50 && risk < 25 && !["Avoid", "Hard Avoid"].includes(stock.conviction);
  });
  return {
    stocks,
...
async function runFullNseScan() {
  ...
  const applyFullScanPayload = (payload) => {
    ...
    lastSuccessfulFetch = Date.now();
    currentDashboardDelay = DASHBOARD_POLL_MS;
    const proxyText = scanMeta.proxy_scored ? `, proxy ${scanMeta.proxy_scored}` : "";
    const basisText = scanMeta.rank_basis === "watch_only_relaxed" ? " Watch-only fallback shown because strict buy gates found no names." : "";
    setScanUi(`Scan complete. Universe ${scanMeta.universe_size || "?"}, passed ${scanMeta.passed_liquidity || "?"}, scored ${scanMeta.total_scored || "?"}${proxyText}, ranked ${scanMeta.ranked_candidates || "?"}.${basisText}`);
  };

  const pollStatus = async () => {
    const status = await getJson("/api/admin/full-nse-scan/status");
    ...
    setScanUi(
      `[${pct}%] ${status.message || "Scanning real NSE universe..."}` +
    ...
    if (status.status === "complete") {
      const result = await getJson("/api/admin/full-nse-scan/result");
  ...
  try {
    ...
    pollTimer = setInterval(() => {
      pollStatus().catch(error => {
        setScanUi(`Status polling failed: ${error.message}`);
      });
    }, 5000);
  } catch (error) {
    setScanUi("");
