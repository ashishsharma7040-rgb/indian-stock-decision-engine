...
function renderEntryPlan(stock, compact = false) {
  ...
  return `<section class="panel entry-panel">
    ...
    <div class="entry-zones">
      <div class="ezone target"><small>Trigger</small><strong>${money(entry.buy_stop_trigger ?? entry.breakout_level)}</strong></div>
...
function renderActionPlan(stock) {
  ...
  const tone = plan.action === "BUY" ? "green" : plan.action === "AVOID" ? "red" : "amber";
  return `<section class="panel action-plan ${tone}">
    <div class="panel-title">Action Plan</div>
    <div class="action-plan-head">
      <div class="action-badge ${tone}">${escapeHtml(plan.action || "WATCH")}</div>
      <div>
    ...
    <div class="action-metrics">
      ${metric("Buy stop", money(plan.trigger_price))}
      ...
      ${metric("Target 1", money(plan.target_1))}
      ${metric("Target 2", money(plan.target_2))}
...
function topCandidatePanel(title, rows, scoreKey) {
  ...
  const statusLine = isWeekly && scanMeta
    ? `Universe ${countText(scanMeta.universe_size)} | Passed ${countText(scanMeta.passed_liquidity)} | Real-history ${countText(scanMeta.real_history_rankable)} | Scored ${countText(scanMeta.total_scored)}`
...
async function runFullNseScan() {
  ...
  const applyFullScanPayload = (payload) => {
    ...
    const basisMore = scanMeta.rank_basis === "await_history_enrichment"
      ? ` ${scanMeta.ranking_warning || "Top picks are blank until real 200+ bar history is available."}`
      : basisText;
...
function drawLightweightChart(target, rows, benchmarkBars = []) {
  ...
  const actionPlan = selectedStock?.action_plan || {};
  ...
  [
    {value: Number(actionPlan.trigger_price || entry.buy_stop_trigger || entry.breakout_level), title: "Trigger", color: "#f59e0b"},
    {value: Number(entry.breakout_level), title: "Breakout", color: "#fbbf24"},
    {value: Number(entry.stop), title: "Stop", color: "#f43f5e"},
    {value: Number(actionPlan.target_1 || entry.target_1), title: "Target 1", color: "#10b981"},
    {value: Number(actionPlan.target_2 || entry.target_2), title: "Target 2", color: "#22c55e"},
