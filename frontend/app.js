...
function renderMarketStrip() {
  ...
  const chips = [
    ["Regime", `<span class="regime-pill ${regimeTone}">${escapeHtml(market.regime || "NA")}</span>`],
    ["Market score", `${market.score ?? "NA"}/100`],
    ["Nifty", money(market.nifty_close || 0)],
    ["Breadth", `${market.breadth_above_50dma ?? "NA"}% adv`],
    ["Live", `<span class="regime-pill ${liveTone}">${live.feed_open ? "Shoonya on" : live.configured ? "Connecting" : "Not configured"}</span>`]
  ];
  ...
  return `<section class="panel desk-card top-candidate-card">
    <div class="panel-headline">
      <div>
        ...
        <div class="status-line">${isWeekly && scanMeta ? `Universe ${scanMeta.universe_size || "?"} | Passed ${scanMeta.passed_liquidity || "?"} | Scored ${scanMeta.total_scored || "?"}` : `Last calculated ${topCandidateState.lastCalculatedAt ? formatIstTime(topCandidateState.lastCalculatedAt) : formatIstTime(dashboard.generated_at)} IST`}</div>
      </div>
...
function normalizeFullScanPayload(payload) {
  const stocks = payload.stocks || payload.focus?.stocks || dashboard?.stocks || [];
  ...
  return {
    stocks,
    topWeekly: (payload.top_weekly || payload.top_weekly_candidates || [...filtered].sort((a, b) => Number(b.weekly_score || 0) - Number(a.weekly_score || 0))).slice(0, 3),
    topMonthly: (payload.top_monthly || payload.top_monthly_candidates || [...filtered].sort((a, b) => Number(b.monthly_score || 0) - Number(a.monthly_score || 0))).slice(0, 3),
    scanMeta: payload.scan_meta || {},
...
async function runFullNseScan() {
  ...
  setNotice("Running full NSE scan. This may take a few minutes; keep this tab open.", "blue");
  const steps = [
    "Refreshing NSE equity universe and bhavcopy...",
    "Applying liquidity, delivery, circuit, and volume filters...",
    "Loading official/EOD price histories...",
    "Running technical, business, event, sector, and risk scoring...",
    "Ranking weekly and monthly candidates...",
  ];
  let step = 0;
  const stepTimer = setInterval(() => {
    setScanUi(steps[Math.min(step, steps.length - 1)]);
    step += 1;
  }, 2500);
  try {
    let payload;
    try {
      payload = await getJson("/api/admin/run-full-nse-scan", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({scan_limit: "all", force: true})
      });
    } catch (fullScanError) {
      setScanUi("Full-scan endpoint unavailable on this deploy. Falling back to focus scan...");
      try { await getJson("/api/universe/refresh", {method: "POST"}); } catch {}
      const fresh = await fetchDashboardPayload(true);
      payload = {
        ...fresh,
        stocks: fresh.stocks || [],
        top_weekly_candidates: fresh.top_weekly_candidates || [],
        top_monthly_candidates: fresh.top_monthly_candidates || [],
        scan_meta: {total_scored: fresh.scored_research_total || fresh.stocks?.length || 0, universe_size: fresh.nse_universe?.total || 0, passed_liquidity: fresh.stocks?.length || 0},
      };
    }
    const normalized = normalizeFullScanPayload(payload);
    if (normalized.stocks.length) dashboard.stocks = normalized.stocks;
    ...
    lastSuccessfulFetch = Date.now();
    currentDashboardDelay = DASHBOARD_POLL_MS;
    setScanUi(`Scan complete. Universe ${scanMeta.universe_size || "?"}, passed ${scanMeta.passed_liquidity || "?"}, scored ${scanMeta.total_scored || "?"}.`);
    clearNotice();
  } catch (error) {
    setScanUi("");
