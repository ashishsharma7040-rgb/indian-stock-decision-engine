...
def business_quality_score(fundamentals: dict[str, Any]) -> dict[str, Any]:
    ...
    if pe is None or growth <= 0:
        ...
        valuation_score = valuation_score * 0.65 + sector_relative_score(to_float(pe), medians.get("pe"), False) * 0.35
        if pe > 80:
            valuation_score -= 20
    ...
    total, breakdown = weighted_score(
        {
            ...
            "roce_roe": ((roce_score + roe_score) / 2, 13),
...
def technical_strength_score(
    ...
    checks = {
        ...
        "idiosyncratic_momentum_positive": float(idio["score"]) > 0.5,
    ...
    if (breakout_fast or breakout_slow or atr_breakout) and vpt_info["score"] < 50:
        ...
        score -= 7
    if (breakout_fast or breakout_slow or atr_breakout) and float(idio["score"]) < 0.5:
    ...
    adv20 = avg_vol
    liquidity_cap_pct = float((event_context or {}).get("liquidity_cap_pct", 0.01))
    liquidity_cap_units = int(adv20 * liquidity_cap_pct) if adv20 else fixed_risk_units
    ...
    return {
        ...
        "entry": {
            ...
            "position_sizing": {
                ...
                "sizing_method": "min(fixed risk, inverse volatility, liquidity cap)",
            },
        },
        "exit_rules": {
...
def risk_penalty(
    ...
    for flag in forensic.get("flags", []):
        if flag == "piotroski_below_7":
            penalties["piotroski_below_7"] = 14
...
def final_decision(
    ...
    technical = technical_strength_score(
        ...
        event_context={
            **event_volume_context(event_result.get("events", [])),
            ...
            "liquidity_cap_pct": company.get("liquidity_cap_pct", 0.01),
        },
    )
    ...
    idiosyncratic_ok = (not breakout_active) or idio_score > 0.5
    portfolio_check = portfolio_concentration_check(company, portfolio_context or {})
    candidate = all(five_questions.values()) and market_result["score"] >= 45 and data_complete_enough and forensic_clean and idiosyncratic_ok
