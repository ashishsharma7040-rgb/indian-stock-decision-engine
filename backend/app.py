...
def score_focus_universe_row(row: dict[str, Any], market_result: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        scored["universe_row"] = row
        scored["data_mode"] = "real_nse_scored"
...
def build_full_nse_scan_payload(scan_limit: Any = 300, force: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        if str(requested_scan_limit).lower() == "all":
            ...
            scan_limit = max(20, min(int(requested_scan_limit or os.getenv("FULL_NSE_SCAN_LIMIT", "300")), max_limit))
        ...
        rows_to_score = UNIVERSE_STORE.top_liquid(limit=scan_limit)
        ...
        set_full_scan_status(
            "running",
            15,
            f"Scoring {total_rows} real investable NSE names with {max_workers} workers.",
            scan_meta={"universe_size": UNIVERSE_STORE.count(), "passed_liquidity": total_rows, "data_mode": "real_nse_only"},
        )
        ...
        ranked = [
            ...
            and stock.get("data_mode") in {"real_nse_scored", "nse_dynamic_real", "nse_dynamic_focus"}
        ]
...
def run_full_nse_scan(
    ...
    background_tasks.add_task(asyncio.to_thread, build_full_nse_scan_payload, scan_limit, force)
    return {
        "status": "started",
