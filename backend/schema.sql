...
alter table companies add column if not exists series text default 'EQ';
alter table companies add column if not exists isin text;
alter table companies add column if not exists is_candidate boolean default false;
alter table companies add column if not exists weekly_score int;
alter table companies add column if not exists monthly_score int;
alter table companies add column if not exists conviction text;
...
create index if not exists idx_price_history_daily_symbol_date
  on price_history_daily(symbol, date desc);
...
create table if not exists scan_runs (
  ...
  ranking_eligible_count int default 0,
  insufficient_history_count int default 0,
  data_quality_failed_count int default 0,
  error text,
  meta jsonb default '{}'::jsonb
);

create index if not exists idx_scan_runs_status_finished
  on scan_runs(status, finished_at desc);
...
create table if not exists scan_results (
  ...
  data_mode text,
  data_quality_gate jsonb default '{}'::jsonb,
...
create index if not exists idx_scan_results_run_rank
  on scan_results(scan_run_id, rank_weekly, rank_monthly);
