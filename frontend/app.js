  ...
  return `<section class="panel desk-card top-candidate-card">
    <div class="panel-headline">
      <div>
        ...
        <div class="status-line">Last calculated ${topCandidateState.lastCalculatedAt ? formatIstTime(topCandidateState.lastCalculatedAt) : formatIstTime(dashboard.generated_at)} IST</div>
      </div>
      <button class="btn blue recalc-btn ${topCandidateState.loading ? "loading" : ""}" data-recalculate-best="1">${topCandidateState.loading ? "Calculating" : "Recalculate Best"}</button>
    </div>
...
function renderHome() {
  ...
  const weeklyRows = topCandidates("weekly_score");
  const monthlyRows = topCandidates("monthly_score");
...
async function refreshTopCandidates() {
  ...
  renderHome();
  setNotice("Recalculating top candidates from the live investable universe...", "blue");
  try {
    if (!isDemo) {
    ...
    dashboard.top_weekly_candidates = topCandidates("weekly_score");
    dashboard.top_monthly_candidates = topCandidates("monthly_score");
    topCandidateState.lastCalculatedAt = new Date().toISOString();
    clearNotice();
  } catch (error) {
    setNotice(`Best-candidate recalculation failed: ${error.message}`, "red");
  } finally {
