...
def bars_payload(symbol: str, period: str) -> dict[str, Any]:
    ...
    payload = {"symbol": symbol.upper(), "period": period.lower(), "bars": normalized, "benchmark": bench_norm, "updated_at": datetime.now(timezone.utc).isoformat()}
...
def dashboard() -> dict[str, Any]:
    ...
    return {
        ...
        "latency_strategy": {
            ...
            "prices": "Dashboard quotes use Shoonya ticks first, then configured price keys, then Yahoo Finance fallback.",
...
def bars_detail(symbol: str, period: str = "3m") -> dict[str, Any]:
    try:
        return bars_payload(symbol, period)
    except Exception as exc:
