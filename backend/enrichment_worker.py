...
async def _existing_adjusted_history_counts(symbols: list[str]) -> dict[str, int]:
    ...
    async with AsyncSessionLocal() as session:
        for index in range(0, len(symbols), chunk_size):
            ...
            result = await session.execute(
                select(PriceHistoryDaily.symbol, func.count(PriceHistoryDaily.id))
                .where(PriceHistoryDaily.symbol.in_(batch), PriceHistoryDaily.is_adjusted.is_(True))
                .group_by(PriceHistoryDaily.symbol)
            )
...
async def _upsert_historical_ohlcv(session: Any, symbol: str, bars: list[dict[str, Any]]) -> int:
    ...
    if not values:
        return 0
    stmt = pg_insert(PriceHistoryDaily).values(values)
...
async def _retained_raw_bhavcopy_dates(session: Any) -> set[date]:
    ...
    result = await session.execute(
        ...
        .limit(max(1, int(os.getenv("BHAVCOPY_RETENTION_DATES", "2"))))
    )
