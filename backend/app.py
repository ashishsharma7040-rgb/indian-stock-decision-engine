...
def full_scan_cache_valid(max_age_seconds: int = 1800) -> bool:
    ...
    if not generated_at or not FULL_NSE_SCAN_CACHE.get("payload"):
        return False
    try:
        age = datetime.now(timezone.utc) - datetime.fromisoformat(str(generated_at).replace("Z", "+00:00"))
...
def build_full_nse_scan_payload(scan_limit: Any = 1000, force: bool = False) -> dict[str, Any]:
    """Run the expensive full-universe scan over the investable NSE cache.

    This is intentionally backend-side and batch-oriented. It first refreshes/imports the
    NSE universe, applies the liquidity/delivery/circuit filters in universe_store, and
    then sends only the investable/top-liquid names through the existing scoring engine.
    """
    max_limit = int(os.getenv("FULL_NSE_SCAN_MAX", "1500"))
    requested_scan_limit = scan_limit
    if not force and full_scan_cache_valid():
        ...
        FULL_NSE_SCAN_STATUS.update(
            {
                ...
                "error": None,
                "progress": 1,
                "message": "Starting full NSE scan.",
                "scan_meta": {},
            }
        )

    try:
        set_full_scan_status("running", 5, "Refreshing NSE universe and latest bhavcopy.")
        if force or not UNIVERSE_STORE.count():
            refresh_nse_universe()
        else:
            ensure_universe_loaded()
        universe_meta = UNIVERSE_STORE.meta()
        investable_count = int(universe_meta.get("investable") or 0)
        if str(requested_scan_limit).lower() == "all":
            scan_limit = max(20, min(investable_count or max_limit, max_limit))
        else:
            scan_limit = max(20, min(int(requested_scan_limit or os.getenv("FULL_NSE_SCAN_LIMIT", "1000")), max_limit))

        set_full_scan_status("running", 12, "Refreshing market regime before scoring.")
        try:
            market_refreshed = refresh_market_data()
            market = market_refreshed.get("market", market_support_score(DATASET["market"]))
        except Exception:
            market = market_support_score(DATASET["market"])

        seed_scored = build_scored_universe()
        seen = {stock["symbol"] for stock in seed_scored}
        rows = UNIVERSE_STORE.top_liquid(limit=scan_limit)
        total_rows = len(rows)
        dynamic_scored: list[dict[str, Any]] = []
        skipped = 0
        set_full_scan_status(
            "running",
            18,
            f"Scoring {total_rows} investable NSE names after liquidity/delivery gates.",
            scan_meta={"universe_size": UNIVERSE_STORE.count(), "passed_liquidity": total_rows},
        )
        ...
        for index, row in enumerate(rows, start=1):
            ...
            if index == 1 or index % 10 == 0 or index == total_rows:
                progress = 18 + int((index / max(total_rows, 1)) * 68)
                set_full_scan_status(
                    "running",
                    progress,
                    f"Scored {index}/{total_rows} investable NSE names.",
                    scan_meta={
                        "universe_size": UNIVERSE_STORE.count(),
        ...
        ranked = [
            ...
            and int(stock.get("weekly_score") or 0) >= 50
        ]
        top_weekly = sorted(ranked, key=lambda stock: int(stock.get("weekly_score") or 0), reverse=True)[:3]
        top_monthly = sorted(ranked, key=lambda stock: int(stock.get("monthly_score") or 0), reverse=True)[:3]
        generated_at = datetime.now(timezone.utc).isoformat()
        scan_meta = {
            "universe_size": UNIVERSE_STORE.count(),
            "passed_liquidity": total_rows,
        ...
        set_full_scan_status("complete", 100, "Full NSE scan complete.", finished_at=generated_at, scan_meta=scan_meta)
        return payload
    except Exception as exc:
...
def run_full_nse_scan(payload: dict[str, Any] | None = Body(default=None)) -> dict[str, Any]:
    payload = payload or {}
    ...
    scan_limit = payload.get("scan_limit") or os.getenv("FULL_NSE_SCAN_LIMIT", "1000")
    force = bool(payload.get("force", False))
    return build_full_nse_scan_payload(scan_limit=scan_limit, force=force)
...
def scheduled_daily_scan() -> dict[str, Any]:
    ...
    if str(os.getenv("ENABLE_DAILY_FULL_SCAN", "")).lower() in {"1", "true", "yes"}:
        full_scan = build_full_nse_scan_payload(scan_limit=os.getenv("FULL_NSE_SCAN_LIMIT", "1000"), force=True)
    return {
