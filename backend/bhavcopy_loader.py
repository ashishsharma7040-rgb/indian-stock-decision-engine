...
from models import Company, DailyOHLCV, Fundamental, PriceHistoryDaily
...
async def update_database_with_bhavcopy(for_date: date | None = None) -> dict[str, Any]:
    ...
    async with AsyncSessionLocal() as session:
        await session.execute(delete(DailyOHLCV).where(DailyOHLCV.date == as_of, DailyOHLCV.is_adjusted.is_(False)))
...
async def database_counts() -> dict[str, Any]:
    ...
    async with AsyncSessionLocal() as session:
        ...
        bars = await session.scalar(select(func.count()).select_from(DailyOHLCV).where(DailyOHLCV.is_adjusted.is_(False)))
        yahoo_history_bars = await session.scalar(select(func.count()).select_from(PriceHistoryDaily))
        enriched_symbols = await session.scalar(
            select(func.count(func.distinct(PriceHistoryDaily.symbol)))
        )
        legacy_adjusted_daily_ohlcv = await session.scalar(
    ...
    return {
        ...
        "price_history_daily": yahoo_history_bars or 0,
        "enriched_ohlcv": yahoo_history_bars or 0,
        "enriched_symbols": enriched_symbols or 0,
        "legacy_adjusted_daily_ohlcv": legacy_adjusted_daily_ohlcv or 0,
...
async def historical_bars_from_db(
    ...
    """Load Yahoo-enriched long history from PriceHistoryDaily for batch scoring."""
    if not DATABASE_ENABLED or AsyncSessionLocal is None:
    ...
    async with AsyncSessionLocal() as session:
        for batch in _chunks(wanted, max(20, int(chunk_size or 250))):
            result = await session.execute(
                ...
                .where(PriceHistoryDaily.symbol.in_(batch), PriceHistoryDaily.date >= start_date)
                .order_by(PriceHistoryDaily.symbol, PriceHistoryDaily.date)
            )
                ...
                bars_by_symbol.setdefault(str(bar.symbol).upper(), []).append(
                    {
                        ...
                        "source": bar.data_source or "Supabase price_history_daily",
                        "data_source": bar.data_source or "Supabase price_history_daily",
    ...
    for symbol, rows in bars_by_symbol.items():
        ...
        chosen = adjusted if adjusted else clean
        if adjusted:
            adjusted_symbols += 1
        trimmed[symbol] = chosen[-days:]
    usable_count = sum(1 for rows in trimmed.values() if len(rows) >= min_bars)
    return {
        "enabled": True,
        "status": "ok",
        "bars_by_symbol": trimmed,
        "symbols_requested": len(wanted),
        "symbols_with_history": usable_count,
        "symbols_with_any_history": len(trimmed),
