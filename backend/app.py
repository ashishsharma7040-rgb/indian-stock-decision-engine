...
try:
    from bhavcopy_loader import database_counts, ensure_recent_bhavcopy_in_db, latest_bhavcopy_rows_from_db, run_eod_update
...
def require_debug_secret(secret: str | None) -> None:
    expected = os.getenv("ADMIN_SECRET", "").strip()
    if not expected:
        raise HTTPException(status_code=403, detail="ADMIN_SECRET is not configured; debug endpoints are disabled.")
    if secret != expected:
        raise HTTPException(status_code=403, detail="Invalid admin secret")


def masked_value(value: str | None, visible: int = 3) -> str | None:
    if not value:
        return None
...
def classify_database_debug_error(message: str) -> str:
    lowered = message.lower()
    if "password authentication failed" in lowered:
        return "Wrong database password in DATABASE_URL, or the password was not URL-encoded correctly."
    if "could not translate host name" in lowered or "name or service not known" in lowered:
        return "Wrong Postgres hostname or DNS cannot resolve it. Copy a fresh Supabase Transaction pooler URI."
    if "remaining connection slots" in lowered:
        return "Connection slots are exhausted. Use Supabase Transaction pooler and keep Render DB_POOL_SIZE small."
    if "timeout" in lowered:
        return "Database connection or statement timed out. Check pooler URL, Supabase health, and Render outbound access."
    if "ssl" in lowered:
        return "SSL setting mismatch. Supabase Postgres URLs should use ssl=require."
    return "Check DATABASE_URL format, Supabase pooler mode, password, and Render environment redeploy."
...
def _run_async_sync(coro: Any) -> Any:
    ...
    thread.start()
    thread.join(timeout=float(os.getenv("ASYNC_SYNC_TIMEOUT_SECONDS", "90")))
    if "error" in result:
        raise result["error"]
...
def dynamic_universe_stock_detail(symbol: str) -> dict[str, Any]:
    ...
    data_mode = "nse_search_official_first_scored"
    if len(bars) < 210:
...
def score_focus_universe_row(row: dict[str, Any], market_result: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        scored["universe_row"] = row
        scored["data_mode"] = "real_nse_scored" if "scan proxy" not in " ".join(notes).lower() else "nse_eod_proxy_scored"
...
def score_full_scan_row_fast(row: dict[str, Any], market_result: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        company = company_from_universe_row(row)
        company["corporate_actions_applied"] = False
        company["tailwind_factors"] = [
            "Full NSE scan uses the latest NSE EOD universe row for fast batch ranking.",
            "Open the stock detail view for deeper provider-backed history, news, and fundamentals.",
        ]
        bars = build_scan_proxy_bars_from_row(row, days=240)
        if len(bars) < 200:
            return None
        benchmark_bars = market_result.get("_benchmark_bars") or DATASET["benchmark_bars"]
        ...
        scored["trade_state"] = advance_trade_state(scored)
        scored["connector_notes"] = [
            "Batch full-scan mode: no per-symbol network calls were made.",
            "Scored from NSE universe/bhavcopy EOD row with deterministic context bars.",
        ]
        scored["universe_row"] = row
        scored["data_mode"] = "nse_eod_batch_scored"
        scored["price_source"] = "NSE bhavcopy EOD"
        scored["price_as_of"] = row.get("as_of")
...
def build_full_nse_scan_payload(scan_limit: Any = "all", force: bool = False) -> dict[str, Any]:
        ...
        FULL_NSE_SCAN_STATUS.update(
            {
                ...
                "error": None,
                "progress": 1,
                "message": "Starting real NSE full scan.",
                "scan_meta": {},
            }
        )
    ...
    try:
        ...
        if not rows_to_score:
            ...
            rows_to_score = [row for row in rows_to_score if row.get("price") is not None]
        ...
        set_full_scan_status(
            "running",
            15,
            f"Scoring {total_rows} NSE names with {max_workers} workers ({scan_filter_mode}).",
            scan_meta={
                "universe_size": UNIVERSE_STORE.count(),
        ...
        def score_one(row: dict[str, Any]) -> dict[str, Any] | None:
            ...
            return score_full_scan_row_fast(dict(row), market)

        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(score_one, row): row for row in rows_to_score}
                    ...
                    if done_count % 5 == 0 or done_count == total_rows:
                        ...
                        set_full_scan_status(
                            ...
                            scan_meta={
                                ...
                                "proxy_so_far": sum(1 for item in all_scored if item.get("data_mode") in {"nse_eod_proxy_scored", "nse_eod_batch_scored"}),
        ...
        proxy_count = sum(1 for stock in summaries if stock.get("data_mode") in {"nse_eod_proxy_scored", "nse_eod_batch_scored"})
        allowed_full_scan_modes = {"real_nse_scored", "nse_eod_proxy_scored", "nse_eod_batch_scored", "nse_dynamic_real", "nse_dynamic_focus"}
        strict_ranked = [
            stock
            ...
            and stock.get("data_mode") in allowed_full_scan_modes
        ]
        ...
        if not ranked and summaries:
            ranked = [
                stock
                for stock in summaries
                if stock.get("data_mode") in allowed_full_scan_modes
            ]
            rank_basis = "best_available_research_only" if ranked else "none"
        def rank_key(stock: dict[str, Any], score_key: str) -> tuple[int, int, int]:
            data_bonus = 3 if stock.get("data_mode") == "real_nse_scored" else 1
            candidate_bonus = 2 if stock.get("candidate") else 0
            return (data_bonus, candidate_bonus, int(stock.get(score_key) or 0))
        ...
        payload = {
            ...
            "latest_critical_events": critical_events(all_scored),
            "sector_map": sector_map(all_scored),
            "stocks": sorted(summaries, key=lambda stock: int(stock.get("weekly_score") or 0), reverse=True),
            "scan_meta": scan_meta,
...
def run_full_nse_scan(
    ...
    FULL_NSE_SCAN_THREAD = threading.Thread(
        target=build_full_nse_scan_payload,
...
@app.get("/api/debug/test-supabase")
async def debug_test_supabase(secret: str | None = None) -> dict[str, Any]:
    """Temporary protected credential/connection diagnostic for Render.

    Remove this endpoint after deployment credentials are confirmed.
    """
    require_debug_secret(secret)
    try:
        from database import DATABASE_ENABLED as db_enabled, database_status as db_status, engine as db_engine
    ...
    public_status = {
        ...
        "url_configured": status.get("url_configured"),
        "url_kind": status.get("url_kind"),
        "url_source": status.get("url_source"),
        "url_redacted": status.get("url_redacted"),
        "host": status.get("host"),
        "port": status.get("port"),
        "database": status.get("database"),
        "username_style": status.get("username_style"),
        "pooler": status.get("pooler"),
        "pooler_mode_hint": status.get("pooler_mode_hint"),
        "connection_warnings": status.get("connection_warnings"),
    }
    if not db_enabled or db_engine is None:
        return {"ok": False, "stage": "configured", "error": status.get("error") or "Engine not initialized", "database": public_status}
    ...
    try:
        ...
        return {
            ...
            "stage": "select_1",
            "error": message,
            "interpretation": classify_database_debug_error(message),
            "database": public_status,
        }


@app.get("/api/debug/test-shoonya")
async def debug_test_shoonya(secret: str | None = None, attempt_start: bool = False) -> dict[str, Any]:
    """Temporary protected Shoonya credential/TOTP diagnostic for Render.

    Does not return password, API key, IMEI, TOTP secret, or generated OTP.
    Remove this endpoint after deployment credentials are confirmed.
    """
    require_debug_secret(secret)
    creds = live_feed._credential_values()
    missing = live_feed.missing_credentials()
    try:
        twofa = live_feed._twofa()
        twofa_error = None
    except Exception as exc:
        twofa = ""
        twofa_error = str(exc)
    ...
    payload: dict[str, Any] = {
        "ok": bool(live_feed.configured and twofa and not twofa_error),
        "configured": live_feed.configured,
        "missing_credentials": missing,
        "credential_flags": {key: bool(value) for key, value in creds.items()},
        "user_id_masked": masked_value(creds.get("user_id")),
        "totp_secret_present": bool(creds.get("totp_secret")),
        "manual_twofa_present": bool(creds.get("twofa")),
        "twofa_valid": bool(twofa and not twofa_error),
        "twofa_length": len(twofa) if twofa else 0,
        "twofa_source": "totp_secret" if creds.get("totp_secret") else "manual_or_runtime_twofa" if creds.get("twofa") else None,
        "twofa_error": twofa_error,
        "host": status.get("host"),
        "websocket": status.get("websocket"),
        "sdk_available": status.get("sdk_available"),
        "feed_status": status.get("status"),
        "feed_open": status.get("feed_open"),
        "last_error": status.get("last_error"),
        "accepted_env_names": status.get("accepted_env_names"),
        "note": "Use attempt_start=true only when you want this debug endpoint to trigger the Shoonya login/feed start.",
    }
    if attempt_start and live_feed.configured:
        live_feed.start()
        await asyncio.sleep(3)
        refreshed = live_feed.status()
        payload.update({
            "attempted_start": True,
            "feed_status": refreshed.get("status"),
            "feed_open": refreshed.get("feed_open"),
            "last_error": refreshed.get("last_error"),
            "sdk_available": refreshed.get("sdk_available"),
        })
    elif attempt_start:
        payload["attempted_start"] = False
        payload["start_blocked_reason"] = "Shoonya credentials are incomplete."
    return payload
