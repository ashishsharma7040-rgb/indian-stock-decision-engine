...
async function getJson(path, options = {}) {
  ...
  try {
    ...
    if (error?.name === "AbortError") {
      throw new Error(`Request timed out after ${Math.round(timeoutMs / 1000)}s. Render may still be waking up.`);
    }
    throw error;
  } finally {
...
async function fetchDashboardPayload(force = false) {
  try {
    const focus = await getJson(`/api/v1/dashboard/focus?scan_limit=60${force ? "&force=true" : ""}`, {timeoutMs: 7000});
    return dashboardFromFocus(focus);
  } catch (focusError) {
    return getJson("/api/dashboard", {timeoutMs: 9000});
  }
}
...
function renderBootShell() {
  ...
  isDemo = true;
  renderAll();
  setNotice(`Loading live backend from ${apiBase}. Render cold starts can take a little while; the app shell is ready now.`, "amber");
}
