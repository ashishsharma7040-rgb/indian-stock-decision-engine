...
let databaseInventoryStatus = null;
...
let settingsActionTone = "blue";
let settingsOtpSubmitting = false;
let settingsLiveRestarting = false;
...
function yesNoChip(value, positive = "Yes", negative = "No") {
  return renderStatusChip(value ? positive : negative, value ? "green" : "red");
}
...
function liveConnectionTone(status = liveDebugStatus || liveStatus || {}) {
  if (isDemo) return "amber";
  if (status.feed_open) return "green";
  if (status.configured && status.status && !["not_connected", "not_configured"].includes(status.status)) return "amber";
  if (backendHealthStatus && backendHealthStatus.status !== "ok") return "red";
  return "blue";
}
function liveConnectionLabel(status = liveDebugStatus || liveStatus || {}) {
  if (backendHealthStatus && backendHealthStatus.status !== "ok") return "Backend Down";
  if (isDemo) return "Demo";
  if (status.feed_open) return "Live";
  if (status.configured && ["logged_in", "websocket_starting", "login_attempting", "sdk_loaded"].includes(status.status)) return "Waiting";
  if (status.configured) return "Delayed";
  return "Waiting";
}
...
function renderTopbarStatus() {
  const pill = qs("liveStatusPill");
  if (!pill) return;
  const label = liveConnectionLabel();
  const tone = liveConnectionTone();
  pill.textContent = label;
  pill.className = `status-chip ${tone}`;
  pill.title = isDemo
    ? "Backend demo fallback is active."
    : (liveDebugStatus?.next_step || liveStatus?.status || "Connection state");
}
...
function renderDataQualityGate(stock) {
  const gate = stock.data_quality_explanation || stock.data_quality_gate || {};
  const tone = gate.pass ? "green" : "amber";
  const issues = [
    ...(stock.data_quality_gate?.price_data_quality?.issues || []),
    ...(gate.price_warnings || stock.data_quality_gate?.price_data_quality?.warnings || []),
    ...(gate.missing_fields || []),
  ].filter(Boolean);
  return `<section class="panel premium-card">
    ...
    <div class="decision-chip-row">
      ...
      ${renderStatusChip(`Completeness ${stock.data_quality_gate?.actual_completeness_pct ?? "NA"}%`, "neutral")}
      ${gate.corporate_actions_applied ? renderStatusChip("Adjusted history", "green") : renderStatusChip("Adjustment status unknown", "amber")}
      ${renderStatusChip(`Bars ${gate.bars_available ?? stock.history_bars_used ?? "NA"}`, "neutral")}
    </div>
    ${(gate.message || gate.warning) ? `<div class="notice notice-amber">${escapeHtml(gate.message || gate.warning)}</div>` : ""}
    <div class="summary-grid">
      ${metric("Recent source", escapeHtml(gate.recent_source || stock.price_source || "NA"))}
      ${metric("Latest bar", escapeHtml(gate.latest_bar_date || stock.price_as_of || "NA"))}
      ${metric("Real vs fallback", escapeHtml(gate.real_vs_fallback || "NA"))}
      ${metric("Imputed fields", gate.imputed_fields?.length ? escapeHtml(gate.imputed_fields.join(", ")) : "None")}
    </div>
...
function renderPositionSizingCard(stock) {
  ...
  return `<section class="panel premium-card">
    ...
    <div class="action-metrics">
      ${metric("Suggested quantity", gatePass ? (plan.suggested_quantity ?? sizing.suggested_quantity ?? "NA") : "WAIT_DATA")}
      ${metric("Risk capital", gatePass ? money(plan.risk_capital ?? sizing.risk_capital) : "WAIT_DATA")}
      ${metric("Approx position value", gatePass ? money(plan.approx_position_value ?? sizing.approx_position_value) : "WAIT_DATA")}
      ${metric("Account size", money(plan.account_size ?? sizing.account_size))}
      ${metric("Risk / share", money(sizing.risk_per_share))}
      ${metric("Stop distance", gatePass ? money(Math.abs(Number((plan.trigger_price ?? stock.entry?.buy_stop_trigger ?? stock.entry?.breakout_level) || 0) - Number((plan.stop ?? stock.entry?.stop) || 0))) : "WAIT_DATA")}
    </div>
...
function renderResearchSummaryCard(stock) {
  const summary = stock.research_summary || {};
  const gatePass = Boolean(stock.data_quality_gate?.pass);
  return `<section class="panel premium-card research-summary-card">
    <div class="panel-title">Research Summary</div>
    <div class="body-text">${escapeHtml(summary.why_interesting || stock.action_plan?.reason_summary || stock.verdict || "Research summary is loading.")}</div>
    ${summary.why_not_actionable ? `<div class="notice notice-amber">${escapeHtml(summary.why_not_actionable)}</div>` : ""}
    <div class="summary-grid">
      ${metric("Main trigger", escapeHtml(summary.main_trigger || "NA"))}
      ${metric("Main risk", escapeHtml(summary.main_risk || "NA"))}
      ${metric("Primary desk", escapeHtml(summary.time_horizon || "Research"))}
      ${metric("Setup state", escapeHtml(summary.setup_state || stock.action_plan?.setup_state || "NA"))}
    </div>
    ${!gatePass ? `<div class="status-line">WAIT_DATA: ranking blocked until data quality improves.</div>` : ""}
  </section>`;
}

function renderShortTermPanel(stock) {
  const shortView = stock.short_term_view || {};
  const rs = shortView.relative_strength_vs_nifty || {};
  const gatePass = Boolean(stock.data_quality_gate?.pass);
  return `<section class="panel premium-card">
    <div class="panel-title">Short-Term Setup</div>
    <div class="action-metrics">
      ${metric("Weekly score", shortView.score ?? stock.weekly_score ?? "NA")}
      ${metric("Technical score", shortView.technical_score ?? stock.technical_strength?.score ?? "NA")}
      ${metric("Breakout level", money(stock.entry?.breakout_level))}
      ${metric("Buy stop trigger", gatePass ? money(shortView.trigger ?? stock.action_plan?.trigger_price) : "WAIT_DATA")}
      ${metric("Pullback zone", zone(shortView.pullback_zone ?? stock.action_plan?.pullback_zone))}
      ${metric("Stop loss", money(shortView.stop ?? stock.action_plan?.stop))}
      ${metric("Target 1", money(shortView.target_1 ?? stock.action_plan?.target_1))}
      ${metric("Target 2", money(shortView.target_2 ?? stock.action_plan?.target_2))}
      ${metric("Risk-reward", shortView.risk_reward_target_1 ?? stock.action_plan?.risk_reward_target_1 ?? "NA")}
      ${metric("Volume confirmation", shortView.volume_confirmation ? "Confirmed" : "Not confirmed")}
      ${metric("RS vs Nifty", rs.state ? `${rs.state}${rs.pct !== undefined && rs.pct !== null ? ` (${rs.pct}%)` : ""}` : "NA")}
      ${metric("Setup state", shortView.setup_state || stock.action_plan?.setup_state || "NA")}
    </div>
    ${!gatePass ? `<div class="status-line">Do not rank/action until data-quality gate passes.</div>` : ""}
  </section>`;
}

function renderLongTermPanel(stock) {
  const longView = stock.long_term_view || {};
  const metricsBlock = longView.metrics || {};
  return `<section class="panel premium-card">
    <div class="panel-title">Long-Term Thesis</div>
    <div class="action-metrics">
      ${metric("Monthly score", longView.score ?? stock.monthly_score ?? "NA")}
      ${metric("Business quality", longView.business_quality_score ?? stock.business_quality?.score ?? "NA")}
      ${metric("Sector tailwind", longView.sector_tailwind_score ?? stock.sector_tailwind?.score ?? "NA")}
      ${metric("ROCE", metricsBlock.roce === undefined || metricsBlock.roce === null ? "NA" : `${metricsBlock.roce}%`)}
      ${metric("ROE", metricsBlock.roe === undefined || metricsBlock.roe === null ? "NA" : `${metricsBlock.roe}%`)}
      ${metric("Sales CAGR", metricsBlock.sales_cagr === undefined || metricsBlock.sales_cagr === null ? "NA" : `${metricsBlock.sales_cagr}%`)}
      ${metric("Profit CAGR", metricsBlock.profit_cagr === undefined || metricsBlock.profit_cagr === null ? "NA" : `${metricsBlock.profit_cagr}%`)}
      ${metric("Debt / equity", metricsBlock.debt_equity ?? "NA")}
      ${metric("PE", metricsBlock.pe ?? "NA")}
    </div>
    <div class="body-text">${escapeHtml(longView.business_quality_explanation || "Business-quality commentary will appear here.")}</div>
    <div class="body-text subtle" style="margin-top:8px">${escapeHtml(longView.sector_tailwind_explanation || "Sector-tailwind commentary will appear here.")}</div>
    ${longView.watchlist_only ? `<div class="notice notice-amber">Suitable only for watchlist until data-quality gate passes.</div>` : ""}
  </section>`;
}

function renderRiskDashboardPanel(stock) {
  const risk = stock.risk_dashboard || {};
  const flags = risk.risk_flags || stock.explanation_json?.risk_flags || [];
  return `<section class="panel premium-card">
    <div class="panel-title">Risk Dashboard</div>
    <div class="summary-grid">
      ${metric("Business risk", escapeHtml(risk.business_risk || "NA"))}
      ${metric("Valuation risk", escapeHtml(risk.valuation_risk || "NA"))}
      ${metric("Technical risk", escapeHtml(risk.technical_risk || "NA"))}
      ${metric("Data risk", escapeHtml(risk.data_risk || "NA"))}
      ${metric("Market risk", escapeHtml(risk.market_risk || "NA"))}
      ${metric("Event risk", escapeHtml(risk.event_risk || "NA"))}
      ${metric("Liquidity risk", escapeHtml(risk.liquidity_risk || "NA"))}
    </div>
    <div class="decision-chip-row">${flags.length ? flags.slice(0, 8).map(flag => renderStatusChip(gateLabel(flag), "amber")).join("") : renderStatusChip("No major rule-based risk flag", "green")}</div>
  </section>`;
}

function renderResearchThesisPanel(stock) {
  const sections = stock.pro_research_sections || {};
  const llmThesis = stock.premium_thesis?.thesis || stock.generated_thesis?.thesis || "";
  if (llmThesis) {
    return `<section class="panel premium-card">
      <div class="panel-title">Research Thesis</div>
      <div class="body-text">${escapeHtml(llmThesis)}</div>
    </section>`;
  }
  return `<section class="panel premium-card">
    <div class="panel-title">Research Thesis</div>
    <div class="grid two thesis-grid">
      <div>
        <div class="section-kicker">Bull case</div>
        ${(sections.bull_case || []).map(line => `<p class="body-text">${escapeHtml(line)}</p>`).join("") || "<div class='status-line'>No bull case yet.</div>"}
      </div>
      <div>
        <div class="section-kicker">Bear case</div>
        ${(sections.bear_case || []).map(line => `<p class="body-text">${escapeHtml(line)}</p>`).join("") || "<div class='status-line'>No bear case yet.</div>"}
      </div>
      <div>
        <div class="section-kicker">Neutral case</div>
        ${(sections.neutral_case || []).map(line => `<p class="body-text">${escapeHtml(line)}</p>`).join("") || "<div class='status-line'>No neutral case yet.</div>"}
      </div>
      <div>
        <div class="section-kicker">Checklist before action</div>
        ${(sections.checklist_before_action || []).map(line => `<div class="check-row"><span class="dot ok"></span><div>${escapeHtml(line)}</div></div>`).join("") || "<div class='status-line'>No checklist loaded.</div>"}
      </div>
    </div>
  </section>`;
}
...
async function refreshLiveDebugStatus(force = false) {
  ...
  try {
    liveDebugStatus = await getJson(`/api/live/debug?t=${Date.now()}`);
  } catch (error) {
    liveDebugStatus = {
      configured: false,
      status: "unavailable",
...
async function refreshBackendHealthStatus(force = false) {
  if (isDemo && !force) {
    backendHealthStatus = {
      status: "demo",
      mode: "demo",
      message: "Backend unreachable; demo fallback is active.",
      api_base: apiBase,
    };
    renderTopbarStatus();
  ...
  try {
    const payload = await getJson(`/api/health?t=${Date.now()}`);
    backendHealthStatus = {
      status: payload.status || "ok",
      mode: apiBase.includes("127.0.0.1") || apiBase.includes("localhost") ? "local" : "deployed",
...
async function refreshProviderStatus(force = false) {
  if (isDemo && !force) return providerStatus;
  try {
    providerStatus = await getJson(`/api/providers/status?t=${Date.now()}`);
  } catch (error) {
    providerStatus = {status: "unavailable", error: error.message};
  }
  if (settingsOpen) renderSettingsPanel();
  return providerStatus;
}

async function refreshRedisStatus(force = false) {
  if (isDemo && !force) return redisStatus;
  try {
    redisStatus = await getJson(`/api/redis/status?t=${Date.now()}`);
  } catch (error) {
    redisStatus = {enabled: false, status: "unavailable", error: error.message};
  }
  if (settingsOpen) renderSettingsPanel();
  return redisStatus;
}

async function refreshDatabaseInventoryStatus(force = false) {
  if (isDemo && !force) return databaseInventoryStatus;
  try {
    databaseInventoryStatus = await getJson(`/api/database/bhavcopy-dates?t=${Date.now()}`);
  } catch (error) {
    databaseInventoryStatus = {enabled: false, error: error.message};
  }
  if (settingsOpen) renderSettingsPanel();
  return databaseInventoryStatus;
}

async function refreshSettingsData(force = false) {
  settingsLoading = true;
  renderSettingsPanel();
  await Promise.allSettled([
    refreshBackendHealthStatus(force),
    refreshLiveDebugStatus(force),
    refreshDatabaseStatus(false),
    refreshDatabaseInventoryStatus(force),
    refreshProviderStatus(force),
    refreshRedisStatus(force),
  ]);
...
async function loadDashboard() {
  try {
    ...
    refreshBackendHealthStatus(false).catch(() => {});
    ...
    isDemo = true;
    refreshBackendHealthStatus(true).catch(() => {});
...
function setSettingsFeedback(message = "", tone = "blue") {
  settingsActionMessage = message || "";
  settingsActionTone = tone;
  if (settingsOpen) renderSettingsPanel();
}
...
function scheduleLiveDebugFollowups() {
  [1200, 4500, 9000].forEach(delay => {
    setTimeout(() => {
      refreshLiveDebugStatus(true).catch(() => {});
      refreshRedisStatus(true).catch(() => {});
    }, delay);
  });
}
...
async function refreshDatabaseStatus(showNotice = false) {
  let status = null;
  try {
    status = await getJson(`/api/database/status?t=${Date.now()}`);
    databaseStatus = {
      ...(databaseStatus || {}),
      ...status,
    ...
    databaseStatus = {
      ...
      status_source: databaseStatus ? "cached_with_error" : "error",
    };
    if (showNotice) setNotice(`Database status refresh failed: ${error.message}`, "amber");
  }
  if (dashboard) dashboard.database = databaseStatus;
  if (showNotice) {
    const view = databaseStatus || status || {};
    const counts = view.counts || {};
    const msg = view.enabled
      ...
      : `Supabase is not connected. ${view.error || view.import_error || view.transient_error || "Add DATABASE_URL in Render."}`;
    setNotice(msg, view.enabled && !counts.bhavcopy_stale ? "blue" : view.url_kind === "supabase_api_url" ? "red" : "amber");
  }
  renderMarketStrip();
  if (!qs("homeView")?.classList.contains("hidden")) renderHome();
  if (settingsOpen) renderSettingsPanel();
...
function filteredStocks() {
  ...
  if (!usingUniverseSearch) {
    if (activeFilter === "triggered") rows = rows.filter(s => isTriggeredStock(s));
    if (activeFilter === "breakout") rows = rows.filter(s => {
      const breakout = Number(s?.entry?.breakout_level);
      const price = Number(s?.price);
      if (!Number.isFinite(breakout) || !Number.isFinite(price) || breakout <= 0) return false;
      const gap = ((breakout - price) / breakout) * 100;
      return gap >= 0 && gap <= 3.5;
    });
    if (activeFilter === "short") rows = rows.filter(s => Number(s.weekly_score || 0) >= Number(s.monthly_score || 0));
    if (activeFilter === "long") rows = rows.filter(s => Number(s.monthly_score || 0) > Number(s.weekly_score || 0) + 4);
    if (activeFilter === "watchlist") {
      const pinned = watchlistSymbols();
      rows = rows.filter(s => pinned.has(s.symbol));
    }
    if (activeFilter === "avoid") rows = rows.filter(s => ["Avoid", "Hard Avoid"].includes(s.conviction) || s.risk_score >= 18 || s.action_plan?.action === "AVOID");
    if (activeFilter === "dataissue") rows = rows.filter(s => s.data_quality_gate && s.data_quality_gate.pass === false);
    if (text) rows = rows.filter(s => `${s.symbol} ${s.name} ${s.sector} ${s.industry}`.toLowerCase().includes(text));
    rows.sort((a, b) => Number(b.weekly_score || 0) - Number(a.weekly_score || 0));
  }
  return rows;
}
...
function renderShoonyaStatusCard(status = {}) {
  ...
  const unresolved = status.unresolved_symbols || [];
  const pending = status.pending_subscription_symbols || [];
  return `<section class="panel desk-card premium-card">
    ...
    ${metric("Trading mode", status.trading_enabled ? "Enabled" : "Market-data-only")}
    ${status.last_login_response_stat ? metric("Login stat", escapeHtml(status.last_login_response_stat)) : ""}
    ${missing.length ? `<div class="status-line">Missing: ${escapeHtml(missing.join(", "))}</div>` : ""}
    ${pending.length ? `<div class="status-line">Pending subscribe: ${escapeHtml(pending.slice(0, 8).join(", "))}</div>` : ""}
    ${unresolved.length ? `<div class="status-line">Unresolved: ${escapeHtml(unresolved.slice(0, 8).join(", "))}</div>` : ""}
    ${status.websocket_last_close_reason ? `<div class="status-line">Socket close: ${escapeHtml(status.websocket_last_close_reason)}</div>` : ""}
    ${status.last_login_error ? `<div class="notice notice-amber">${escapeHtml(status.last_login_error)}</div>` : ""}
...
function backendFallbackPanel() {
  if (!isDemo) return "";
  const message = backendHealthStatus?.message || "Backend is unreachable right now.";
  return `<section class="panel premium-card backend-fallback-card">
    <div class="panel-title">Backend Unreachable</div>
    <div class="body-text">The frontend is running in demo fallback while the live backend is waking up or unavailable.</div>
    <div class="decision-chip-row">
      ${renderStatusChip("Demo fallback active", "amber")}
      ${renderStatusChip(apiBase, "neutral")}
    </div>
    <div class="status-line">${escapeHtml(message)}</div>
    <div class="card-actions">
      <button class="btn blue" data-refresh-dashboard="1">Retry dashboard</button>
      <button class="btn" data-open-settings="1">Open Settings</button>
    </div>
  </section>`;
}

function renderSettingsChecklistRow(label, ok, detail = "") {
  return `<div class="settings-check-row ${ok ? "pass" : "fail"}">
    <span class="dot ${ok ? "ok" : "warn"}"></span>
    <div>
      <strong>${escapeHtml(label)}</strong>
      ${detail ? `<div class="subtle">${escapeHtml(detail)}</div>` : ""}
    </div>
    ${renderStatusChip(ok ? "OK" : "Check", ok ? "green" : "amber")}
  </div>`;
}

function renderSettingsSection(title, kicker, body, actions = "") {
  return `<section class="panel premium-card settings-card">
    <div class="panel-headline">
      <div>
...
function renderSettingsPanel() {
  ...
  const live = liveDebugStatus || {};
  const db = databaseStatus || {};
  const dbCounts = db.counts || {};
  const dbInventory = databaseInventoryStatus || {};
  const provider = providerStatus || {};
  const redis = redisStatus || {};
  const missing = live.missing_credentials || [];
  const unresolved = live.unresolved_symbols || [];
  const pending = live.pending_subscription_symbols || [];
  const blockers = live.likely_blockers || [];
  const credentialDiagnostics = live.credential_diagnostics || {};
  const credentialAssessment = live.credential_assessment || [];
  const loginSucceeded = live.last_login_response_stat === "Ok" || live.status === "logged_in" || live.feed_open;
  const loginDetail = loginSucceeded
    ? `Last accepted ${live.last_login_success_at ? formatIstTime(live.last_login_success_at) : "recently"}${live.totp_source ? ` via ${live.totp_source}` : ""}.`
    : (live.last_login_error || live.last_error || "No successful login recorded yet.");
  const otpReady = Boolean(live.credential_flags?.totp_secret || live.credential_flags?.twofa);
  const otpDetail = live.totp_source === "runtime_twofa"
    ? `Runtime OTP override submitted${live.runtime_twofa_last_used_at ? `; last used ${formatIstTime(live.runtime_twofa_last_used_at)}` : ""}.`
    : live.runtime_twofa_set_at
      ? `Runtime OTP saved at ${formatIstTime(live.runtime_twofa_set_at)} and will override the Render TOTP secret for the next login attempt.`
      : (live.totp_source || "No runtime OTP source seen yet.");
  const inventoryDates = dbInventory.dates || dbCounts.bhavcopy_dates || [];
  const providerRows = [
    ["NSE bhavcopy", dbCounts.latest_bhavcopy_date ? "Live" : "Unknown", dbCounts.latest_bhavcopy_date ? `Latest ${dbCounts.latest_bhavcopy_date}` : "Run Sync bhavcopy"],
    ["Yahoo fallback", dbCounts.enriched_ohlcv ? "Fallback ready" : "Awaiting", dbCounts.enriched_ohlcv ? `${countText(dbCounts.enriched_ohlcv)} history rows stored` : "Run Sync Yahoo data"],
    ["News providers", provider.latest_quote_sources ? "Available" : "Unknown", provider.error || "Backend decides provider mix"],
    ["Corporate actions", "Conditional", "Applied when backend history adjustment is available"],
    ["Fundamentals enrichment", dbCounts.fundamental_snapshots ? "Available" : "Partial", dbCounts.fundamental_snapshots ? `${countText(dbCounts.fundamental_snapshots)} snapshots` : "Manual/Supabase merge still needed for some names"],
  ];
  ...
  body.innerHTML = `
    <div class="settings-grid">
      ${renderSettingsSection(
        ...
        `
          <div class="decision-chip-row">
            ${renderStatusChip(health.mode === "local" ? "Local mode" : "Deployed mode", "blue")}
            ${renderStatusChip(health.status === "ok" ? "Backend reachable" : health.status === "demo" ? "Demo fallback" : "Backend down", health.status === "ok" ? "green" : health.status === "demo" ? "amber" : "red")}
          </div>
          ${renderSettingsChecklistRow("Backend reachable", health.status === "ok", health.message || "")}
          <label class="settings-field">Current API base<input class="input" id="settingsApiBaseInput" value="${escapeHtml(apiBase)}"></label>
          <div class="status-line">Last successful dashboard fetch ${lastSuccessfulFetch ? timeAgo(lastSuccessfulFetch) : "not yet"}.</div>
        `,
        `<div class="card-actions"><button class="btn blue" data-test-backend="1">Test backend</button><button class="btn" id="settingsSaveApiBaseBtn">Save backend URL</button></div>`
      )}
      ${renderSettingsSection(
        "Live Feed Diagnostics",
        "Shoonya market-data-only mode",
        `
          <div class="decision-chip-row">
            ${renderStatusChip(live.feed_open ? "Live" : live.configured ? "Configured / waiting" : "Not configured", live.feed_open ? "green" : live.configured ? "amber" : "red")}
            ${renderStatusChip(live.trading_enabled ? "Trading enabled" : "Trading disabled", live.trading_enabled ? "red" : "green")}
            ${renderStatusChip(`Status ${live.status || "unknown"}`, "neutral")}
          </div>
          ${renderSettingsChecklistRow("Shoonya credentials configured", Boolean(live.configured), missing.length ? `Missing: ${missing.join(", ")}` : "Credential categories look present.")}
          ${renderSettingsChecklistRow("TOTP / OTP available", otpReady, otpDetail)}
          ${renderSettingsChecklistRow("SDK installed", live.sdk_available !== false, live.sdk_available === false ? "Broker SDK import failed or Python runtime mismatch." : "SDK import looks healthy.")}
          ${renderSettingsChecklistRow("Login successful", loginSucceeded, loginDetail)}
          ${renderSettingsChecklistRow("WebSocket open", Boolean(live.feed_open), live.websocket_last_close_reason || "Waiting for feed_open=true.")}
          ${renderSettingsChecklistRow("Symbols resolved", !unresolved.length, unresolved.length ? `Unresolved: ${unresolved.join(", ")}` : `${countText(live.resolved_tokens_count || 0)} tokens resolved.`)}
          <div class="settings-kv-grid">
            ${metric("Configured", live.configured ? "Yes" : "No")}
            ${metric("Started", live.started ? "Yes" : "No")}
            ${metric("Feed open", live.feed_open ? "Yes" : "No")}
            ${metric("Subscribed count", countText(live.subscribed_count || 0))}
            ${metric("Last login attempt", live.last_login_attempt_at ? formatIstTime(live.last_login_attempt_at) : "NA")}
            ${metric("Last login success", live.last_login_success_at ? formatIstTime(live.last_login_success_at) : "NA")}
            ${metric("Socket opened", live.websocket_opened_at ? formatIstTime(live.websocket_opened_at) : "NA")}
            ${metric("Attempt age", live.login_attempt_age_seconds != null ? `${countText(live.login_attempt_age_seconds)}s` : "NA")}
            ${metric("Login timeout", live.login_timeout_seconds ? `${countText(live.login_timeout_seconds)}s` : "NA")}
            ${metric("Password mode", live.last_login_password_mode || credentialDiagnostics.password_mode_setting || "raw")}
          </div>
          ${live.last_endpoint_host ? `<div class="status-line">Endpoint host: ${escapeHtml(live.last_endpoint_host)}</div>` : ""}
          ${live.last_endpoint_websocket ? `<div class="status-line">Endpoint websocket: ${escapeHtml(live.last_endpoint_websocket)}</div>` : ""}
          <div class="settings-kv-grid" style="margin-top:12px">
            ${metric("User ID shape", credentialDiagnostics.user_id?.present ? `${countText(credentialDiagnostics.user_id.length)} chars` : "Missing")}
            ${metric("Vendor code shape", credentialDiagnostics.vendor_code?.present ? `${countText(credentialDiagnostics.vendor_code.length)} chars` : "Missing")}
            ${metric("API key shape", credentialDiagnostics.api_key?.present ? `${countText(credentialDiagnostics.api_key.length)} chars` : "Missing")}
            ${metric("IMEI shape", credentialDiagnostics.imei?.present ? `${countText(credentialDiagnostics.imei.length)} chars` : "Missing")}
            ${metric("Runtime OTP age", live.runtime_twofa_age_seconds != null ? `${countText(live.runtime_twofa_age_seconds)}s` : "NA")}
            ${metric("Attempted password modes", (live.attempted_password_modes || []).join(", ") || "NA")}
          </div>
          ${live.status === "login_attempting" && live.login_attempt_age_seconds > ((live.login_timeout_seconds || 35) + 5) ? `<div class="notice notice-red">The broker login call appears stuck beyond the expected timeout. This is usually a broker SDK/network issue rather than a bad click in Settings.</div>` : ""}
          ${pending.length ? `<div class="notice notice-blue">Pending subscription queue: ${escapeHtml(pending.join(", "))}. These are waiting for the WebSocket to open and are not treated as token failures yet.</div>` : ""}
          ${live.last_error ? `<div class="notice notice-amber">${escapeHtml(live.last_error)}</div>` : ""}
          ${live.last_login_error ? `<div class="notice notice-amber">${escapeHtml(live.last_login_error)}</div>` : ""}
          ${credentialAssessment.length ? `<div class="what-next-box"><div class="section-kicker">Credential Assessment</div><ul class="tight-list">${credentialAssessment.map(item => `<li><strong>${escapeHtml(item.label)}:</strong> ${escapeHtml(item.detail)}</li>`).join("")}</ul></div>` : ""}
          ${blockers.length ? `<div class="what-next-box"><div class="section-kicker">Most Likely Blockers</div><ul class="tight-list">${blockers.map(item => `<li>${escapeHtml(item)}</li>`).join("")}</ul></div>` : ""}
          <div class="what-next-box">
            <div class="section-kicker">What to Fix Next</div>
            <div class="body-text">${escapeHtml(live.next_step || "Refresh diagnostics to inspect the next blocker.")}</div>
          </div>
          <section style="margin-top:12px">
            <div class="section-kicker">Runtime OTP</div>
            <div class="status-line">Use this only when you do not have a working TOTP secret on Render. The value is sent to backend only, overrides the TOTP secret for the next login attempt, and is never stored in the frontend.</div>
            <div class="status-line">Shoonya OTP itself must come from Shoonya / Prism / your registered channel. This app can accept the code, but it cannot trigger the broker to send the OTP to your phone.</div>
            <input class="input" id="settingsShoonyaOtpInput" inputmode="numeric" autocomplete="one-time-code" placeholder="Current Shoonya OTP / TOTP">
          </section>
          ${settingsActionMessage ? `<div class="notice notice-${escapeHtml(settingsActionTone)}" style="margin-top:12px">${escapeHtml(settingsActionMessage)}</div>` : ""}
        `,
        `<div class="card-actions"><button class="btn blue" data-refresh-live-debug="1">Refresh live debug</button><button class="btn ${settingsLiveRestarting ? "loading" : ""}" data-restart-live="1">${settingsLiveRestarting ? "Restarting..." : "Start / retry live feed"}</button><button class="btn ${settingsOtpSubmitting ? "loading" : ""}" id="settingsSubmitShoonyaOtpBtn">${settingsOtpSubmitting ? "Submitting..." : "Submit OTP"}</button></div>`
      )}
      ${renderSettingsSection(
        "Database / Supabase",
        "Persistence",
        `
          <div class="decision-chip-row">
            ${renderStatusChip(db.enabled ? "Configured" : "Not configured", db.enabled ? "green" : "red")}
            ${renderStatusChip(`Connection ${db.connection_test || (db.transient_error ? "cached" : "unknown")}`, db.connection_test === "ok" ? "green" : db.connection_test === "failed" ? "red" : "amber")}
          </div>
          ${renderSettingsChecklistRow("Database enabled", Boolean(db.enabled), db.error || db.import_error || "")}
          ${renderSettingsChecklistRow("Connection test", db.connection_test === "ok", db.connection_error || db.count_error || db.transient_error || "Backend connection test will surface here.")}
          ${renderSettingsChecklistRow("Bhavcopy inventory loaded", Boolean(inventoryDates.length), dbInventory.error || (inventoryDates.length ? `Latest ${inventoryDates[0]}` : "No retained bhavcopy dates reported yet."))}
          <div class="settings-kv-grid">
            ${metric("Bhavcopy rows", countText(dbCounts.daily_ohlcv || 0))}
            ${metric("History rows", countText(dbCounts.enriched_ohlcv || 0))}
            ${metric("History symbols", countText(dbCounts.enriched_symbols || 0))}
            ${metric("Bhavcopy latest", dbCounts.latest_bhavcopy_date || "NA")}
            ${metric("Stale", dbCounts.bhavcopy_stale ? "Yes" : "No")}
            ${metric("Latest age (days)", dbInventory.latest_age_days ?? dbCounts.latest_bhavcopy_age_days ?? "NA")}
            ${metric("Retention dates", countText(dbInventory.retention_dates || dbCounts.bhavcopy_retention_dates || 0))}
          </div>
          ${inventoryDates.length ? `<div class="status-line">Retained bhavcopy dates: ${escapeHtml(inventoryDates.slice(0, 6).join(", "))}</div>` : ""}
          ${db.transient_error ? `<div class="notice notice-amber">Using last known database status while refresh retries: ${escapeHtml(db.transient_error)}</div>` : ""}
          ${db.connection_warnings?.length ? `<div class="notice notice-amber">${escapeHtml(db.connection_warnings.join(" | "))}</div>` : ""}
        `,
        `<div class="card-actions"><button class="btn" data-refresh-db-status="1">Refresh DB status</button><button class="btn blue" data-sync-bhavcopy="1">Sync bhavcopy</button><button class="btn" data-sync-yahoo="1">Sync Yahoo</button></div>`
      )}
      ${renderSettingsSection(
        "Redis / Tick Cache",
        "Live cache",
        `
          <div class="decision-chip-row">
            ${renderStatusChip(redis.enabled ? "Configured" : "Not configured", redis.enabled ? "green" : "amber")}
            ${renderStatusChip(redis.status || (redis.enabled ? "Connected" : "Unavailable"), redis.enabled ? "green" : "amber")}
          </div>
          <div class="status-line">${escapeHtml(redis.error || redis.detail || "Not configured or not exposed by backend.")}</div>
        `,
        `<div class="card-actions"><button class="btn" data-refresh-redis-status="1">Refresh Redis</button></div>`
      )}
      ${renderSettingsSection(
        "Data Providers",
        "Source map",
        `
          <div class="provider-grid">
            ${providerRows.map(([label, state, note]) => `
              <div class="provider-tile">
                <strong>${escapeHtml(label)}</strong>
                ${renderStatusChip(state, state.toLowerCase().includes("live") ? "green" : state.toLowerCase().includes("fallback") ? "amber" : state.toLowerCase().includes("partial") ? "amber" : "neutral")}
                <div class="subtle">${escapeHtml(note)}</div>
              </div>
            `).join("")}
          </div>
        `,
        `<div class="card-actions"><button class="btn" data-refresh-provider-status="1">Refresh providers</button><button class="btn blue" data-market-refresh="1">Refresh market</button></div>`
      )}
      ${renderSettingsSection(
        "Safe Manual Actions",
        "Operator tools",
        `
          <div class="status-line">Use these only to refresh research data and diagnostics. Market-data-only mode remains enforced.</div>
        `,
        `<div class="card-actions"><button class="btn blue" data-refresh-dashboard="1">Refresh dashboard</button><button class="btn" data-recalculate-best="1">Run full scan</button><button class="btn" data-alert-scan="1">Scan alerts</button></div>`
      )}
    </div>
    ${settingsLoading ? `<div class="settings-loading-strip">Refreshing connection health...</div>` : ""}`;
}
...
function loadingStockSkeleton(symbol) {
  return `<section class="panel skeleton-stock">
    ...
    <div class="loading-stage-list">
      ...
      <span>Applying corporate actions</span>
...
function renderHome() {
  ...
  const triggeredRows = (dashboard.stocks || [])
    ...
    .slice(0, 5);
  const nearBreakoutRows = (dashboard.stocks || [])
    .filter(stock => {
      const breakout = Number(stock?.entry?.breakout_level);
      const price = Number(stock?.price);
      if (!Number.isFinite(breakout) || !Number.isFinite(price) || breakout <= 0) return false;
    ...
    .sort((a, b) => (b.weekly_score || 0) - (a.weekly_score || 0))
    .slice(0, 5);
  const avoidRows = (dashboard.avoid_list || []).slice(0, 5);
  const watchRows = readWatchlist().map(item => stockBySymbol(item.symbol)).filter(Boolean).slice(0, 5);
  const dataIssueRows = (dashboard.stocks || []).filter(stock => stock.data_quality_gate?.pass === false).slice(0, 6);
  const alerts = dashboard.latest_critical_events || [];
  const weeklyRows = (scanWeeklyResults || topCandidates("weekly_score")).slice(0, 5);
  const monthlyRows = (scanMonthlyResults || topCandidates("monthly_score")).slice(0, 5);
  const focusRows = (dashboard.stocks || []).slice(0, 12);
  const liveCard = renderShoonyaStatusCard(liveDebugStatus || dashboard.live_feed || liveStatus || {});
  ...
  qs("homeView").innerHTML = `
    <div class="home-head desk-home-head">
      ...
      <div class="desk-actions">
        ...
        <button class="btn" data-open-settings="1">Settings</button>
        ${yahooEnrichStatusText ? `<div class="sync-status-text">${escapeHtml(yahooEnrichStatusText)}</div>` : ""}
      </div>
    </div>
    ${backendFallbackPanel()}
    ${dataPrepPanel()}
    ${renderHomeDataQualityBanner()}
    <div class="home-top-grid">
      <section class="panel desk-card">
        <div class="panel-title">Triggered Now</div>
        ${candidateRows(triggeredRows, "weekly_score")}
      </section>
      ${topCandidatePanel("Top Short-Term Setups", weeklyRows, "weekly_score")}
      ${topCandidatePanel("Top Long-Term Research Ideas", monthlyRows, "monthly_score")}
      ${renderMarketHealthCard(market)}
      ${renderScanProgressPanel(scanMeta || dashboard.scan_meta || {})}
      ${liveCard}
    </div>
    <div class="home-mid-grid">
      <section class="panel desk-card premium-card">
        <div class="panel-title">Near Breakout</div>
        ${candidateRows(nearBreakoutRows, "weekly_score")}
      </section>
      <section class="panel desk-card premium-card">
        <div class="panel-title">Watchlist</div>
        ${watchRows.length ? candidateRows(watchRows, "weekly_score") : "<div class='empty-state'>Pin names from the sidebar or stock page to build your watchlist.</div>"}
      </section>
    </div>
    <div class="home-mid-grid">
      <section class="panel desk-card premium-card">
        <div class="panel-title">Avoid / Risk List</div>
        ${avoidRows.length ? candidateRows(avoidRows, "weekly_score") : "<div class='empty-state'>No major avoid candidates right now.</div>"}
      </section>
      <section class="panel desk-card premium-card">
        <div class="panel-title">Data Quality Warnings</div>
        ${dataIssueRows.length ? dataIssueRows.map(stock => `<button class="candidate-row desk-row" data-symbol="${stock.symbol}">
          <div>
            <strong>${escapeHtml(stock.symbol)}</strong>
            <span>${escapeHtml(stock.data_quality_gate?.warning || "WAIT_DATA: ranking blocked until data quality improves")}</span>
          </div>
          <div class="candidate-tags">${renderStatusChip("WAIT_DATA", "amber")}</div>
        </button>`).join("") : "<div class='status-line'>No active data-quality warnings in the focus list.</div>"}
      </section>
    </div>
    <div class="home-mid-grid">
      <section class="panel">
        <div class="panel-title">Sector Strength Map</div>
...
function buildFallbackDetail(summary) {
  ...
  const gatePass = Boolean(summary.data_quality_gate?.pass);
  const action = gatePass && summary.candidate ? "WATCH" : "WAIT_DATA";
  const reasonSummary = summary.action_plan?.reason_summary || summary.verdict || (gatePass ? "Research setup under review." : "WAIT_DATA: ranking blocked until data quality improves.");
  return {
    ...summary,
    ...
    research_summary: summary.research_summary || {
      why_interesting: `This is a cached summary for ${summary.symbol}. Open the full live detail to confirm the current setup.`,
      why_not_actionable: gatePass ? null : "WAIT_DATA: ranking blocked until data quality improves.",
      main_trigger: summary.entry?.breakout_level ? `Watch ${money(summary.entry.breakout_level)} as the main confirmation level.` : "No trigger loaded.",
      main_risk: (summary.risk_flags || [])[0] || "Risk review still pending.",
      time_horizon: Number(summary.monthly_score || 0) > Number(summary.weekly_score || 0) + 4 ? "Long-term research" : "Short-term swing",
      setup_state: summary.trade_state?.state || "Screened",
    },
    short_term_view: summary.short_term_view || {
      score: summary.weekly_score,
      technical_score: summary.technical_score,
      setup_state: summary.trade_state?.state || "Screened",
      volume_confirmation: false,
      relative_strength_vs_nifty: {state: "Cached summary", pct: null},
      trigger: summary.entry?.buy_stop_trigger || summary.entry?.breakout_level,
      pullback_zone: summary.entry?.pullback,
      stop: summary.entry?.stop,
      target_1: summary.entry?.target_1,
      target_2: summary.entry?.target_2,
      risk_reward_target_1: summary.entry?.risk_reward_target_1,
      risk_reward_target_2: summary.entry?.risk_reward_target_2,
    },
    long_term_view: summary.long_term_view || {
      score: summary.monthly_score,
      business_quality_score: summary.business_score,
      sector_tailwind_score: summary.tailwind_score,
      metrics: {},
      business_quality_explanation: "Detailed business-quality commentary loads with full stock detail.",
      sector_tailwind_explanation: "Detailed sector-tailwind commentary loads with full stock detail.",
      watchlist_only: !gatePass,
    },
    risk_dashboard: summary.risk_dashboard || {
      business_risk: "Review",
      valuation_risk: "Review",
      technical_risk: "Needs full chart detail",
      data_risk: gatePass ? "Normal" : "Data gate blocked",
      market_risk: dashboard?.market_regime?.regime || "Unknown",
      event_risk: "Review",
      liquidity_risk: "Review",
      risk_flags: summary.risk_flags || [],
    },
    pro_research_sections: summary.pro_research_sections || {
      bull_case: ["Cached summary is available; open full detail to confirm the latest setup and context."],
      bear_case: [!gatePass ? "Data-quality gate is still blocked." : "Fresh provider detail is still loading."],
      neutral_case: ["Treat this as research support, not a guaranteed action call."],
      checklist_before_action: ["Refresh the stock detail before acting.", "Confirm data-quality gate and stop level."],
    },
    data_quality_explanation: summary.data_quality_explanation || {
      pass: gatePass,
      message: summary.data_quality_gate?.warning || "Cached summary data is being shown.",
      missing_fields: summary.data_quality_gate?.missing_fields || [],
      imputed_fields: [],
      bars_available: summary.history_bars_used || 0,
      corporate_actions_applied: summary.data_quality_gate?.corporate_actions_applied,
      recent_source: summary.price_source || summary.data_mode || "Cached summary",
      latest_bar_date: summary.price_as_of || null,
      real_vs_fallback: gatePass ? "Research summary from engine cache" : "Fallback / incomplete",
      price_warnings: summary.data_quality_gate?.price_data_quality?.warnings || [],
    },
    bars
  };
}
...
function renderStockView(stock) {
  ...
  const gatePass = Boolean(full.data_quality_gate?.pass);
  const actionLabel = actionDisplayLabel(full.action_plan?.action, gatePass);
  const sourceLabel = full.live_source ? "Live" : full.price_source || full.data_mode || "Research";
  qs("stockView").innerHTML = `
    <div class="stock-head terminal-head">
      ...
      <div class="price-block ${full._pulse ? `pulse-${full._pulse}` : ""}">
        ...
        <div class="status-line">Short-term ${full.weekly_score ?? "NA"} | Long-term ${full.monthly_score ?? "NA"} | Decision ${escapeHtml(actionLabel)} | Source ${escapeHtml(sourceLabel)}</div>
      </div>
    </div>
  ...
  return `
    <div class="stock-workspace-grid">
      <div class="stock-left-col">
        ${renderResearchSummaryCard(stock)}
        <section class="panel premium-card">
          <div class="panel-title">Pro Chart</div>
        ...
        ${renderResearchThesisPanel(stock)}
        <section class="panel premium-card">
...
async function submitShoonyaOtp() {
  if (settingsOtpSubmitting) return;
  const twofa = qs("settingsShoonyaOtpInput")?.value.trim() || qs("shoonyaOtpInput")?.value.trim();
  if (!twofa) {
    qs("offlineNotice").textContent = "Enter the current Shoonya OTP first.";
    qs("offlineNotice").classList.remove("hidden");
    setSettingsFeedback("Enter the current Shoonya OTP or TOTP first.", "amber");
    return;
  }
  settingsOtpSubmitting = true;
  setSettingsFeedback("Submitting OTP to backend and restarting Shoonya login...", "blue");
  if (settingsOpen) renderSettingsPanel();
  qs("offlineNotice").textContent = "Sending Shoonya OTP to backend...";
  qs("offlineNotice").classList.remove("hidden");
  try {
    const payload = await getJson("/api/live/twofa", {
      method: "POST",
      headers: {"Content-Type":"application/json"},
      body: JSON.stringify({twofa})
    });
    liveStatus = payload;
    liveDebugStatus = payload;
    const input = qs("settingsShoonyaOtpInput");
    if (input) input.value = "";
    const legacyInput = qs("shoonyaOtpInput");
    if (legacyInput) legacyInput.value = "";
    const message = payload.otp_submission?.message || `Shoonya status: ${payload.status || "unknown"}`;
    setSettingsFeedback(message, "blue");
    qs("offlineNotice").textContent = message;
    renderMarketStrip();
    if (!qs("homeView")?.classList.contains("hidden")) renderHome();
  } catch (error) {
    qs("offlineNotice").textContent = `Shoonya OTP login failed: ${error.message}`;
    setSettingsFeedback(`Shoonya OTP login failed: ${error.message}`, "red");
  } finally {
    settingsOtpSubmitting = false;
    if (settingsOpen) renderSettingsPanel();
  }
}
...
document.addEventListener("click", event => {
  if (event.target.closest("[data-open-settings]") || event.target.id === "settingsBtn") {
    event.preventDefault();
    openSettingsPanel();
    return;
  }
  if (event.target.closest("[data-close-settings]") || event.target.id === "closeSettingsBtn") {
    event.preventDefault();
    closeSettingsPanel();
    return;
  }
  ...
  if (event.target.id === "submitShoonyaOtpBtn" || event.target.id === "settingsSubmitShoonyaOtpBtn") submitShoonyaOtp();
    if (event.target.closest("[data-test-backend]")) refreshBackendHealthStatus(true);
    if (event.target.closest("[data-refresh-live-debug]")) {
      setSettingsFeedback("Refreshing live diagnostics...", "blue");
      refreshLiveDebugStatus(true).then(() => setSettingsFeedback("Live diagnostics refreshed.", "blue")).catch(error => setSettingsFeedback(`Live diagnostics failed: ${error.message}`, "red"));
    }
    if (event.target.closest("[data-refresh-db-status]")) {
      setSettingsFeedback("Refreshing database diagnostics...", "blue");
      Promise.allSettled([refreshDatabaseStatus(true), refreshDatabaseInventoryStatus(true)]).then(() => {
        setSettingsFeedback("Database diagnostics refreshed.", "blue");
      }).catch(error => setSettingsFeedback(`Database diagnostics failed: ${error.message}`, "red"));
    }
    if (event.target.closest("[data-refresh-provider-status]")) refreshProviderStatus(true);
    if (event.target.closest("[data-refresh-redis-status]")) refreshRedisStatus(true);
    if (event.target.closest("[data-restart-live]")) {
      settingsLiveRestarting = true;
      setSettingsFeedback("Requesting Shoonya restart...", "blue");
      if (settingsOpen) renderSettingsPanel();
      getJson("/api/live/restart", {method: "POST"})
        .then(payload => {
          liveDebugStatus = payload;
          liveStatus = payload;
          setNotice(`Shoonya restart requested. Status ${payload.status || "unknown"}.`, payload.feed_open ? "blue" : "amber");
          setSettingsFeedback(payload.feed_open ? "Shoonya feed is live." : `Shoonya restart requested. Current status: ${payload.status || "unknown"}.`, payload.feed_open ? "blue" : "amber");
          renderTopbarStatus();
          renderHome();
          if (settingsOpen) renderSettingsPanel();
          scheduleLiveDebugFollowups();
        })
        .catch(error => {
          setNotice(`Shoonya restart failed: ${error.message}`, "red");
          setSettingsFeedback(`Shoonya restart failed: ${error.message}`, "red");
        })
        .finally(() => {
          settingsLiveRestarting = false;
          if (settingsOpen) renderSettingsPanel();
        });
      return;
    }
  ...
  if (event.target.id === "saveApiBaseBtn" || event.target.id === "settingsSaveApiBaseBtn") {
    apiBase = (qs("settingsApiBaseInput")?.value.trim() || qs("apiBaseInput")?.value.trim() || "http://127.0.0.1:8000");
