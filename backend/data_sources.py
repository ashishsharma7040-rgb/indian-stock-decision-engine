...
def fetch_yahoo_quote(symbol: str, ttl_seconds: int = 600) -> dict[str, Any]:
    now = time.time()
    clean = symbol.upper()
    ...
    latest = bars[-1]
    previous = next((bar for bar in reversed(bars[:-1]) if float(bar.get("close", 0) or 0) > 0), None)
    close = float(latest["close"])
    prev_close = float(previous["close"]) if previous else close
    quote = {
        "symbol": clean,
        "price": round(close, 2),
        ...
        "source": "Yahoo Finance quote cache",
    }
