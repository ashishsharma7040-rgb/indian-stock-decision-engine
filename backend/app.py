...
def score_full_scan_row_fast(
    ...
    try:
        ...
        scored["ranking_eligible"] = data_mode == "supabase_yahoo_history_scored"
        scored["scan_confidence"] = "high" if scored["ranking_eligible"] else "low"
        if not scored["ranking_eligible"]:
            research_only_warning = "Research only: full-universe top picks require 200+ stored Supabase Yahoo bars for this symbol."
            scored["candidate"] = False
            scored["conviction"] = "Insufficient Data"
            scored["verdict"] = research_only_warning
            action_plan = dict(scored.get("action_plan") or {})
            action_plan.update({
                "action": "WAIT_DATA",
                "confidence": "low",
                "summary": research_only_warning,
                "verdict": research_only_warning,
            })
            scored["action_plan"] = action_plan
            gate = scored.setdefault("data_quality_gate", {})
            gate["pass"] = False
            gate["warning"] = research_only_warning
            price_quality = gate.setdefault("price_data_quality", {})
            warnings = list(price_quality.get("warnings") or [])
            if research_only_warning not in warnings:
                warnings.append(research_only_warning)
            price_quality["warnings"] = warnings
            notes.append(research_only_warning)
            scored["trade_state"] = advance_trade_state(scored)
...
def build_full_nse_scan_payload(scan_limit: Any = "all", force: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            for batch_start in range(0, total_rows, scan_batch_size):
                ...
                if not deep_mode:
                    ...
                    batch_prefetch = prefetch_full_scan_db_batch(batch_symbols, history_days=260, history_min_bars=200)
                    ...
                    history_meta["symbols_with_history"] = int(history_meta.get("symbols_with_history") or 0) + len(batch_history_by_symbol)
                ...
                for future in concurrent.futures.as_completed(futures):
                    ...
                    with status_lock:
                        ...
                        if result is not None and not result.get("is_seed_demo"):
                            ...
                            if data_mode in {"nse_eod_proxy_scored", "nse_eod_batch_scored"}:
                                proxy_scored += 1
        ...
        proxy_count = proxy_scored
        enriched_count = enriched_history_scored
        real_history_modes = {"real_nse_scored", "supabase_yahoo_history_scored", "nse_dynamic_real"}
        rankable_real = [
            stock
            for stock in summaries
            if stock.get("ranking_eligible", stock.get("data_mode") in real_history_modes)
        ...
        scan_meta = {
            ...
            "proxy_excluded_from_top_picks": proxy_count,
            ...
            "ranking_warning": None if ranking_ready else "Top picks stay blank until enough symbols have 200+ stored Supabase Yahoo bars. Proxy-scored names are kept for research only.",
        }
        payload = {
            "status": "success",
