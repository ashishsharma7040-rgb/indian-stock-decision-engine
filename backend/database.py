...
except Exception:  # pragma: no cover - SQLAlchemy optional in demo mode
    DBAPIError = InterfaceError = OperationalError = None  # type: ignore
...
def db_retry(max_retries: int = 3, delay: float = 1.0):
    """Retry short DB operations when Render/Supabase drops a socket mid-flight."""
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            retryable_types = tuple(
                item for item in (DBAPIError, InterfaceError, OperationalError, OSError, RuntimeError) if item is not None
            )
            for attempt in range(max(1, int(max_retries))):
                try:
                    return await func(*args, **kwargs)
                except retryable_types as exc:  # type: ignore[misc]
                    last_exc = exc
                    if attempt >= max_retries - 1 or not is_transient_db_error(exc):
                        raise
...
DATABASE_POOL_STRATEGY = "disabled"
SCHEMA_READY = False
...
if not DATABASE_URL:
    ...
    try:
        ...
        use_null_pool = os.getenv("DB_USE_NULL_POOL", "true").lower() in {"1", "true", "yes"}
        ...
        if use_null_pool:
            # This app intentionally bridges async DB calls from sync worker
            # threads during full scans. asyncpg connections are tied to the
            # event loop that created them, so a shared pool can later reuse a
            # connection bound to a closed thread loop and raise
            # "Event loop is closed". NullPool opens a fresh connection for
            # each short DB operation and avoids cross-loop reuse.
            engine_kwargs["poolclass"] = NullPool
            DATABASE_POOL_STRATEGY = "null_pool_loop_safe"
        else:
            ...
            DATABASE_POOL_STRATEGY = "queue_pool"
        engine = create_async_engine(_async_database_url(DATABASE_URL), **engine_kwargs)
...
async def init_db() -> dict[str, Any]:
    ...
    if SCHEMA_READY:
        return {"enabled": True, "status": "already_initialized"}
...
def database_status() -> dict[str, Any]:
    ...
    return {
        ...
        "pool_strategy": DATABASE_POOL_STRATEGY,
        "loop_safe_null_pool": DATABASE_POOL_STRATEGY == "null_pool_loop_safe",
