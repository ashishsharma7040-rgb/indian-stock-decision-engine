...
function renderStockView(stock) {
  ...
  if (activeTab === "chart") setTimeout(() => drawChart(full.bars || [], "priceChart"), 0);
  if (activeTab === "overview") setTimeout(() => drawChart(full.bars || [], "overviewChart"), 0);
}
...
function renderTab(stock) {
  ...
  if (activeTab === "chart") return `<section class="panel"><div class="section-kicker">Price confirmation</div><div class="chart-wrap"><canvas id="priceChart"></canvas></div><div class="chart-legend"><span class="legend-price">Price</span><span class="legend-ema">20 EMA</span><span class="legend-dma">200 DMA</span><span class="legend-breakout">Breakout</span><span class="legend-stop">Stop</span></div></section>`;
...
function drawChart(bars, canvasId = "priceChart") {
  ...
  const rows = (bars || []).slice(-240).filter(bar => Number.isFinite(Number(bar.close)));
  ...
  const top = 28;
  const bottom = 52;
  const priceBottom = rect.height - bottom;
  ...
  ctx.fillText(money(min), left, priceBottom + 18);
...
window.addEventListener("resize", () => {
  if (activeTab === "chart" && selectedStock) drawChart(selectedStock.bars || [], "priceChart");
  if (activeTab === "overview" && selectedStock) drawChart(selectedStock.bars || [], "overviewChart");
});
loadDashboard();
