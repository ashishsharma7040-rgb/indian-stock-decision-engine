...
except Exception:  # pragma: no cover - optional NSE provider layer
    nse_fetcher = None  # type: ignore
...
def request_bytes(url: str, timeout: int = 18) -> bytes:
    ...
    if "nseindia.com" in url:
        try:
            ...
            # Some archive hosts are static and do not need cookies. Fall back to
            # a plain request so a cookie/bootstrap failure does not mask them.
            pass
    request = urllib.request.Request(url, headers=headers)
    with urllib.request.urlopen(request, timeout=timeout) as response:
        return response.read()
...
def request_json_with_headers(url: str, headers: dict[str, str], timeout: int = 12) -> dict[str, Any]:
...
def _raw_yahoo_value(payload: dict[str, Any], *path: str) -> Any:
    current: Any = payload
    for key in path:
...
def fetch_yahoo_fundamentals(symbol: str) -> dict[str, Any]:
    """Best-effort no-key Yahoo quoteSummary fundamentals.

    Yahoo coverage for NSE names is uneven and unofficial. This function never
    invents values; it returns only fields Yahoo explicitly provides.
    """
    yahoo_symbol = yahoo_chart_symbol(symbol)
    query_symbol = urllib.parse.quote(yahoo_symbol, safe="")
    modules = "price,defaultKeyStatistics,financialData,summaryDetail"
    url = f"https://query1.finance.yahoo.com/v10/finance/quoteSummary/{query_symbol}?modules={modules}"
    try:
        payload = cached(f"yahoo-fundamentals:{yahoo_symbol}", 86400, lambda: request_json(url, timeout=15))
        rows = payload.get("quoteSummary", {}).get("result") or []
        if not rows:
            return {}
        row = rows[0]
        market_cap = _raw_yahoo_value(row, "price", "marketCap")
        roe = _raw_yahoo_value(row, "financialData", "returnOnEquity")
        debt_equity = _raw_yahoo_value(row, "financialData", "debtToEquity")
        current_price = _raw_yahoo_value(row, "price", "regularMarketPrice")
        ebitda = _raw_yahoo_value(row, "financialData", "ebitda")
        operating_cash_flow = _raw_yahoo_value(row, "financialData", "operatingCashflow")
        free_cash_flow = _raw_yahoo_value(row, "financialData", "freeCashflow")
        result = {
            "market_cap_cr": round(float(market_cap) / 10_000_000, 2) if market_cap else None,
            "pe": _raw_yahoo_value(row, "summaryDetail", "trailingPE") or _raw_yahoo_value(row, "defaultKeyStatistics", "trailingPE"),
            "forward_pe": _raw_yahoo_value(row, "summaryDetail", "forwardPE") or _raw_yahoo_value(row, "defaultKeyStatistics", "forwardPE"),
            "pb": _raw_yahoo_value(row, "defaultKeyStatistics", "priceToBook"),
            "roe": round(float(roe) * 100, 2) if roe is not None and abs(float(roe)) <= 5 else roe,
            "debt_equity": debt_equity,
            "ebitda": ebitda,
            "operating_cash_flow": operating_cash_flow,
            "cfo_pat": None,
            "fcf_trend": "positive" if free_cash_flow and float(free_cash_flow) > 0 else "negative" if free_cash_flow else None,
            "price": current_price,
            "source": "Yahoo Finance quoteSummary",
            "fundamentals_as_of": datetime.now(timezone.utc).date().isoformat(),
        }
...
def fetch_nse_live_quote(symbol: str) -> dict[str, Any]:
    if nse_fetcher is None:
        raise RuntimeError("NSE free live provider layer is unavailable")
    return nse_fetcher.get_live_quote(symbol)
...
def fetch_market_quote(symbol: str, ttl_seconds: int = 60, bypass_cache: bool = False) -> dict[str, Any]:
    clean = symbol.upper()
    data_pref = env_first("DATA_SOURCE_PREFERENCE").lower()
    provider_pref = env_first("PRICE_DATA_PROVIDER", "FINANCIAL_DATA_PROVIDER").lower()
    cache_key = f"best:{data_pref or 'auto'}:{provider_pref or 'auto'}:{clean}"
    now = time.time()
    hit = _PRICE_CACHE.get(cache_key)
    if not bypass_cache and hit and now - hit[0] < ttl_seconds:
        return hit[1]
    providers = {
        "nsefin": fetch_nse_live_quote,
        "nse": fetch_nse_live_quote,
        "nseazy": fetch_nse_live_quote,
    ...
    elif data_pref in {"nsefin", "nse", "nseazy"}:
        order = [
...
def _parse_bhavcopy_csv(raw_text: str, as_of: date, source: str) -> list[dict[str, Any]]:
    ...
    for row in _read_csv_rows(raw_text):
        ...
        delivery_qty = _clean_float(_pick(row, ["DELIV_QTY", "DELIVERY_QTY", "DeliverableQty", "DELIVERY_QUANTITY"]))
        delivery_pct = _clean_float(_pick(row, ["DELIV_PER", "DELIVERY_PCT", "DeliverablePercent", "DELIVERY_PERCENTAGE"]))
        ...
        upper_circuit = _clean_float(_pick(row, ["UPPER_CIRCUIT", "UPPER_PRICE_BAND", "upperPriceBand", "UPPERBAND"]))
        lower_circuit = _clean_float(_pick(row, ["LOWER_CIRCUIT", "LOWER_PRICE_BAND", "lowerPriceBand", "LOWERBAND"]))
        circuit_limit_pct = None
        if upper_circuit and lower_circuit and close:
            upper_pct = abs(upper_circuit - close) / close * 100
            lower_pct = abs(close - lower_circuit) / close * 100
            circuit_limit_pct = round(min(upper_pct, lower_pct), 2)
        rows.append(
            {
                "symbol": symbol,
                "series": series or "EQ",
                ...
                "delivery_qty": int(delivery_qty) if delivery_qty is not None else None,
                "delivery_pct": round(float(delivery_pct), 2) if delivery_pct is not None else None,
...
def fetch_latest_nse_bhavcopy(max_lookback_days: int | None = None) -> dict[str, Any]:
    if max_lookback_days is None:
        max_lookback_days = int(os.getenv("NSE_BHAVCOPY_MAX_LOOKBACK_DAYS", "20"))
    data_pref = env_first("DATA_SOURCE_PREFERENCE").lower()
    if nse_fetcher is not None and data_pref in {"", "auto", "nsefin", "nse"}:
        try:
...
def fetch_official_ohlcv(symbol: str, days: int = 260) -> list[dict[str, Any]]:
    """Fetch official NSE EOD bars from bhavcopy when the optional nsefin layer works."""
    if nse_fetcher is None:
        return []
    data_pref = env_first("DATA_SOURCE_PREFERENCE").lower()
    if data_pref not in {"", "auto", "nsefin", "nse"}:
        return []
    try:
        return nse_fetcher.get_ohlcv_for_symbol(symbol, days=days)
    except Exception:
        return []


async def fetch_corporate_actions_for_symbol(symbol: str) -> list[dict[str, Any]]:
    if nse_fetcher is None:
        return []
    return await nse_fetcher.fetch_corporate_actions(symbol)


async def fetch_free_fundamentals(symbol: str) -> dict[str, Any]:
    if nse_fetcher is None:
        return {}
    return await nse_fetcher.fetch_fundamentals(symbol)


def fetch_nifty500_breadth(symbols: list[str] | None = None) -> dict[str, Any]:
    if nse_fetcher is None:
        raise RuntimeError("NSE breadth layer is unavailable")
    return nse_fetcher.compute_nifty500_breadth(symbols=symbols)
...
def classify_nse_filing(text: str) -> dict[str, Any]:
    """Deterministic classifier for Indian exchange filings.

    Corporate filings are short, formal, and often misleading for generic NLP.
    A rules-first classifier is safer than VADER for exchange announcements.
    """
    lowered = clean_filing_text(text).lower()
    patterns: list[tuple[str, str, int, str, list[str]]] = [
        ("promoter_buying", "Promoter Buying", 88, "Bullish", ["open market purchase", "promoter purchase", "promoter bought", "acquisition of shares by promoter"]),
        ("order_win", "Order Win", 85, "Bullish", ["order win", "letter of intent", "loi", "awarded", "contract awarded", "purchase order", "work order"]),
        ("rating_upgrade", "Credit Upgrade", 78, "Bullish", ["rating upgraded", "upgrade in credit rating", "revised from", "positive outlook"]),
        ("earnings", "Earnings", 70, "Neutral/Bullish", ["financial results", "quarterly results", "audited results", "profit after tax", "profit"]),
        ("pledge", "Pledge/Governance", 18, "Bearish", ["pledge", "encumbrance", "invocation of pledge"]),
        ("management_exit", "Management Exit", 16, "Bearish", ["resignation", "ceased to be", "steps down", "vacate office"]),
        ("default", "Default", 8, "Bearish", ["default", "delay in payment", "non-payment", "downgrade", "insolvency", "nclt"]),
        ("fraud", "Fraud/Investigation", 5, "Bearish", ["fraud", "forensic audit", "investigation", "search and seizure", "show cause notice"]),
    ]
    for category, label, score, sentiment_label, words in patterns:
        if any(word in lowered for word in words):
            return {
                ...
                "sentiment": max(-0.7, min(0.7, (score - 50) / 50 * 0.7)),
            }
    return {"score": 50, "type": "Routine", "category": "routine", "sentiment_label": "Neutral", "sentiment": 0.0}
...
def estimate_news_sentiment(text: str, source_type: str = "", category: str = "") -> float:
    ...
    if category == "promoter_buying":
        return 0.68
    if source_type in {"exchange_filing", "company_ir", "earnings_transcript"}:
        return float(classify_nse_filing(cleaned)["sentiment"])
...
def rss_items_to_events(xml_text: str, source_name: str, max_records: int = 10) -> list[dict[str, Any]]:
    ...
    for item in root.findall(".//item")[:max_records]:
        ...
        lowered = text.lower()
        category = "promoter_buying" if (
            "promoter" in lowered
            and any(term in lowered for term in ["buy", "bought", "purchase", "acquire", "open market"])
        ) else "news"
        source_type = "promoter_buying" if category == "promoter_buying" else "credible_news"
        sentiment = estimate_news_sentiment(text, source_type=source_type, category=category)
        events.append(
            {
                "title": title,
                "source": source_name,
                ...
                "sentiment": sentiment,
                "importance": 80 if category == "promoter_buying" else 44,
                "timestamp": published,
                "url": link,
                "category": category,
            }
            ...
            bars.append(
                {
                    ...
                    "source": "Alpha Vantage daily adjusted",
                }
            )
...
def fetch_gdelt_news(query: str, max_records: int = 10) -> list[dict[str, Any]]:
    ...
    for article in articles:
        ...
        if len(raw_seen) >= 14 and raw_seen[:14].isdigit():
            try:
                seen_at = datetime.strptime(raw_seen[:14], "%Y%m%d%H%M%S").replace(tzinfo=timezone.utc).isoformat()
            except ValueError:
                seen_at = article.get("seendate")
        events.append(
            {
...
def fetch_newsapi(query: str, max_records: int = 10, bypass_cache: bool = False) -> list[dict[str, Any]]:
    ...
    payload = cached(f"newsapi:{query}:{max_records}", 300, lambda: request_json_with_headers(url, {"X-Api-Key": api_key}), bypass=bypass_cache)
    events: list[dict[str, Any]] = []
    ...
    return [
        {
            ...
            "primary": "NSE equity master plus daily bhavcopy EOD cache, with nsefin as the first adapter when installed",
            "free_tier": "No key; official public NSE files give symbol, close, volume, turnover, and circuit context",
            "production_upgrade": "Licensed NSE data vendor for guaranteed uptime and redistribution rights",
        },
        {
            "layer": "Price OHLCV",
            "primary": "Official NSE bhavcopy via nsefin for daily bars, Yahoo Finance chart only as a fallback",
            "free_tier": "No key; bhavcopy is official EOD, live quotes use nsefin/nseazy best-effort public NSE wrappers",
            "production_upgrade": "Licensed Indian feed such as Global Datafeeds, TrueData, or broker market data API",
        },
        {
            "layer": "Market breadth",
            "primary": "Nifty 500/liquid-universe above-50-DMA breadth computed from bhavcopy, with Nifty 50 advance/decline fallback",
        ...
        {
            "layer": "Fundamentals",
            "primary": "niftyterminal when available, plus Screener CSV/manual import for verification",
            "free_tier": "No key, but response shapes vary; the engine flags missing fields instead of inventing fundamentals",
