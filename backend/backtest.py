...
def run_backtest(
    ...
    if not pit_enabled and not dataset.get("allow_lookahead_backtest"):
        return {
            "config": {
                "horizon_days": horizon_days,
                "target_pct": target_pct,
            ...
            "signals": [],
            "refused": True,
            "warning": "Backtest refused: point-in-time fundamentals are required. Provide fundamentals_snapshots with knowledge_date <= each signal date, or explicitly set allow_lookahead_backtest=true for a smoke test with biased current fundamentals.",
        }
