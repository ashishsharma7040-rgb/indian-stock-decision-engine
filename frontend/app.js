...
function renderTab(stock) {
  ...
  if (activeTab === "chart") return `<section class="panel"><div class="chart-head"><div><div class="section-kicker">Price confirmation</div><div class="status-line">Interactive candles, EMAs, volume, RS vs Nifty, breakout and stop.</div></div><div class="timeframe-row">${["1w","1m","3m","6m","1y"].map(period => `<button class="timeframe-btn ${activeChartPeriod === period ? "active" : ""}" data-chart-period="${period}">${period.toUpperCase()}</button>`).join("")}</div></div><div class="chart-wrap"><div id="priceChart" class="tv-chart"></div></div><div id="chartTooltip" class="chart-tooltip hidden"></div><div class="chart-legend"><span class="legend-price">Candles</span><span class="legend-ema">20 EMA</span><span class="legend-dma">200 DMA</span><span class="legend-rs">RS vs Nifty</span><span class="legend-breakout">Breakout</span><span class="legend-stop">Stop</span></div></section>`;
...
async function drawChart(bars, canvasId = "priceChart", benchmarkBars = []) {
  ...
  if (canvasId === "priceChart" && selectedStock?.symbol && !isDemo) {
    try {
      const payload = await getJson(`/api/bars/${selectedStock.symbol}?period=${activeChartPeriod}`);
