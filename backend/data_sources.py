...
def estimate_news_sentiment(text: str) -> float:
    if _SENTIMENT_ANALYZER:
        return max(-0.7, min(0.7, _SENTIMENT_ANALYZER.polarity_scores(text)["compound"]))
    lowered = text.lower()
    sentiment = 0.0
...
def rss_items_to_events(xml_text: str, source_name: str, max_records: int = 10) -> list[dict[str, Any]]:
    ...
    for item in root.findall(".//item")[:max_records]:
        ...
        sentiment = estimate_news_sentiment(f"{title}. {description}")
        events.append(
            {
                "title": title,
                "source": source_name,
