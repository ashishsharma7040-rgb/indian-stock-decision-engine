...
function scoreTile(label, value, subtitle, options = {}) {
  ...
  return `<div class="stile ${tone} ${options.risk ? "risk" : ""}">
    <div class="stile-label">${escapeHtml(label)}</div>
...
function candidateRows(rows, scoreKey) {
  return (rows || []).slice(0, 3).map(stock => {
    const state = stateName(stock);
    return `<button class="candidate-row desk-row ${isTriggeredStock(stock, state) ? "triggered" : ""}" data-symbol="${stock.symbol}">
      <div>
        <strong>${escapeHtml(stock.symbol)}</strong>
        <span>${escapeHtml(stock.sector || stock.name || "")}</span>
      </div>
      ${stateBadgeHtml(stock, state)}
      <b class="${scoreClass(stock[scoreKey])}">${stock[scoreKey] ?? "NA"}</b>
    </button>`;
...
function renderHome() {
  ...
  qs("homeView").innerHTML = `
    ...
    <div class="home-top-grid">
      ...
      <section class="panel desk-card">
        <div class="panel-title">Top 3 Weekly</div>
    ...
    <div class="home-mid-grid">
      ...
      <section class="panel">
        ...
        <div class="event-list compact">${alerts.slice(0, 3).map(eventHtml).join("") || "<div class='status-line'>No critical events loaded.</div>"}</div>
      </section>
    </div>
  ...
  qs("stockView").innerHTML = `
    <div class="stock-head terminal-head">
      <div>
        ...
        <div class="headline-badges">${badgeForConviction(full.conviction)}${stateBadgeHtml(full, state)}<span class="pill ${full.candidate ? "green" : "amber"}">Candidate ${full.candidate ? "Yes" : "No"}</span>${confidencePill(full.confidence_interval)}</div>
      </div>
...
function renderEvents(stock) {
  ...
  return `<section class="panel">
    ...
    <div id="eventsList" class="event-list">${events.map(eventHtml).join("") || "<div class='status-line'>No events loaded.</div>"}</div>
  </section>`;
}
...
function eventHtml(event) {
  ...
  return `<div class="event ${type}">
    <div class="event-top">
      ...
      <span class="sentiment-chip ${type === "pos" ? "green" : type === "neg" ? "red" : "amber"}">${event.net_score ?? event.importance ?? "NA"}</span>
    </div>
  </div>`;
}
