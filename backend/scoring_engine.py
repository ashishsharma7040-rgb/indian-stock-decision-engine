...
def business_data_quality(fundamentals: dict[str, Any]) -> dict[str, Any]:
    ...
    imputed_fields = [field for field in fundamentals.get("_median_imputed_fields", []) if field in required]
    missing = [field for field in required if fundamentals.get(field) is None]
    raw_missing = sorted({*missing, *imputed_fields})
    completeness = round((len(required) - len(missing)) / len(required) * 100, 1)
    raw_completeness = round((len(required) - len(raw_missing)) / len(required) * 100, 1)
    warning_parts: list[str] = []
    if imputed_fields:
        warning_parts.append(f"Sector-median fallback used for: {', '.join(imputed_fields)}")
    if missing:
        warning_parts.append("Some fields are still missing; neutral defaults were used")
    return {
...
def apply_sector_median_fallbacks(fundamentals: dict[str, Any]) -> dict[str, Any]:
    normalized = dict(fundamentals or {})
    medians = normalized.get("sector_medians") or {}
    imputed_fields = list(normalized.get("_median_imputed_fields") or [])
    seen = set(imputed_fields)
    for field in SECTOR_MEDIAN_FALLBACK_FIELDS:
        if normalized.get(field) is None and medians.get(field) is not None:
            normalized[field] = medians.get(field)
            if field not in seen:
                imputed_fields.append(field)
                seen.add(field)
    normalized["_median_imputed_fields"] = imputed_fields
    normalized["_median_fallback_used"] = bool(imputed_fields)
    return normalized
...
def enhanced_data_quality_check(
    ...
    missing = [field for field in required if fundamentals.get(field) is None]
    imputed_core = [field for field in fundamentals.get("_median_imputed_fields", []) if field in required]
    if len(missing) >= 3:
        issues.append(f"Core fundamentals missing: {', '.join(missing)}")
    elif missing:
        warnings.append(f"Core fundamentals missing: {', '.join(missing)}")
    if imputed_core:
        warnings.append(f"Core fundamentals backfilled from sector medians: {', '.join(imputed_core)}")
...
def validate_scan_history(symbol: str, bars: list[dict[str, Any]], latest_eod_date: Any = None) -> dict[str, Any]:
    required_bars = 220
    ordered = sorted(
        [dict(bar) for bar in bars if bar.get("datetime") or bar.get("date")],
        key=lambda item: str(item.get("datetime") or item.get("date") or ""),
    )
    latest_bar_date = None
    try:
        latest_bar_date = parse_bar_datetime((ordered[-1].get("datetime") or ordered[-1].get("date"))) if ordered else None
    except Exception:
        latest_bar_date = None
    try:
        latest_eod = parse_bar_datetime(latest_eod_date) if latest_eod_date else None
    except Exception:
        latest_eod = None

    checks: dict[str, Any] = {
        "required_bars": required_bars,
        "latest_eod_date": latest_eod.date().isoformat() if latest_eod else None,
        "bars_available": len(ordered),
        "latest_bar_date": latest_bar_date.date().isoformat() if latest_bar_date else None,
        "zero_or_negative_closes": 0,
        "zero_volume_days_last_60": 0,
        "max_daily_jump_pct": None,
        "ohlc_invalid_rows": 0,
    ...
    if len(ordered) < required_bars:
        return {
            "pass": False,
            "reason": f"Need {required_bars}+ valid real daily bars before this stock can be ranked.",
            "bars_available": len(ordered),
            "latest_bar_date": checks["latest_bar_date"],
            "checks": checks,
        }

    sample = ordered[-required_bars:]
    zero_or_negative_closes = 0
    zero_volume_days = 0
    max_daily_jump_pct = 0.0
    invalid_ohlc_rows = 0
    ...
    for idx, bar in enumerate(sample):
        ...
        if close is None or close <= 0:
            zero_or_negative_closes += 1
        if idx >= len(sample) - 60 and volume <= 0:
            zero_volume_days += 1
        if high is None or low is None or high < low:
            invalid_ohlc_rows += 1
        elif close is not None and (close < low * 0.97 or close > high * 1.03):
            invalid_ohlc_rows += 1
        elif open_price is not None and (open_price < low * 0.97 or open_price > high * 1.03):
            invalid_ohlc_rows += 1
        if previous_close and close and previous_close > 0:
            jump_pct = abs(close - previous_close) / previous_close * 100
            max_daily_jump_pct = max(max_daily_jump_pct, jump_pct)
        previous_close = close

    checks["zero_or_negative_closes"] = zero_or_negative_closes
    checks["zero_volume_days_last_60"] = zero_volume_days
    checks["max_daily_jump_pct"] = round(max_daily_jump_pct, 2)
    checks["ohlc_invalid_rows"] = invalid_ohlc_rows

    if latest_eod and latest_bar_date and latest_bar_date.date() < latest_eod.date():
        checks["latest_bar_fresh"] = False
        return {
            "pass": False,
            "reason": f"Stored history is stale versus latest official EOD date {latest_eod.date().isoformat()}.",
            "bars_available": len(ordered),
            "latest_bar_date": checks["latest_bar_date"],
            "checks": checks,
        }
    if zero_or_negative_closes:
        return {
            "pass": False,
            "reason": f"Found {zero_or_negative_closes} zero/negative close values in the last {required_bars} bars.",
            "bars_available": len(ordered),
            "latest_bar_date": checks["latest_bar_date"],
            "checks": checks,
        }
    if zero_volume_days > 10:
        return {
            "pass": False,
            "reason": f"Found {zero_volume_days} zero-volume days in the last 60 bars.",
            "bars_available": len(ordered),
            "latest_bar_date": checks["latest_bar_date"],
            "checks": checks,
        }
    if max_daily_jump_pct > 55:
        return {
            "pass": False,
            "reason": f"Detected an abnormal single-day price jump of {round(max_daily_jump_pct, 2)}%, likely requiring split/bonus normalization.",
            "bars_available": len(ordered),
            "latest_bar_date": checks["latest_bar_date"],
            "checks": checks,
        }
    if invalid_ohlc_rows:
        return {
            "pass": False,
            "reason": f"Found {invalid_ohlc_rows} OHLC rows that failed sanity checks.",
            "bars_available": len(ordered),
            "latest_bar_date": checks["latest_bar_date"],
            "checks": checks,
        }
    return {
        "pass": True,
        "reason": "Validated 220+ real daily bars for scan ranking.",
        "bars_available": len(ordered),
        "latest_bar_date": checks["latest_bar_date"],
        "checks": checks,
    }
...
def forensic_earnings_quality(fundamentals: dict[str, Any]) -> dict[str, Any]:
    ...
    pbt = to_float(fundamentals.get("pbt") or fundamentals.get("profit_before_tax"))
    cash_tax_paid = to_float(fundamentals.get("cash_tax_paid"))
    ...
    total_debt = to_float(fundamentals.get("total_debt"))
    dividends_paid = to_float(fundamentals.get("dividends_paid"), 0)
    gross_block = to_float(fundamentals.get("gross_block") or fundamentals.get("fixed_assets") or fundamentals.get("gross_fixed_assets"))
    cwip = to_float(fundamentals.get("cwip") or fundamentals.get("capital_work_in_progress"))
    sloan_ratio = None
    ...
    cash_tax_rate = None
    if cash_tax_paid is not None and pbt is not None and pbt > 0:
        cash_tax_rate = abs(cash_tax_paid) / pbt
    debt_to_cfo = None
    if total_debt is not None and cfo is not None and cfo > 0:
        debt_to_cfo = total_debt / cfo
    cwip_to_gross = None
    if cwip is not None and gross_block is not None and gross_block > 0:
        cwip_to_gross = cwip / gross_block
    fcf = to_float(fundamentals.get("fcf") or fundamentals.get("free_cash_flow"))
    ...
    metric_scores = {
        ...
        "cash_tax_fidelity": score_between(cash_tax_rate, 0.12, 0.24) if cash_tax_rate is not None else 50,
        "debt_to_cfo": score_inverse(debt_to_cfo, 1.5, 6.5) if debt_to_cfo is not None else 50,
        "cwip_bloat": score_inverse(cwip_to_gross, 0.05, 0.40) if cwip_to_gross is not None else 50,
    ...
    total, breakdown = weighted_score(
        {
            "sloan_ratio": (metric_scores["sloan_ratio"], 14),
            "cfo_to_ebitda": (metric_scores["cfo_to_ebitda"], 14),
            "cash_tax_fidelity": (metric_scores["cash_tax_fidelity"], 12),
            "debt_to_cfo": (metric_scores["debt_to_cfo"], 12),
            "cwip_bloat": (metric_scores["cwip_bloat"], 10),
            "working_capital_divergence": (metric_scores["working_capital_divergence"], 12),
            "cash_conversion_cycle": (metric_scores["cash_conversion_cycle"], 8),
            "altman_z": (metric_scores["altman_z"], 8),
            "piotroski_f": (metric_scores["piotroski_f"], 12),
            "beneish_m": (metric_scores["beneish_m"], 8),
        }
    )
    flags: list[str] = []
    warnings: list[str] = []
    ...
    if cash_tax_rate is not None and pbt is not None and pbt > 0 and cash_tax_rate < 0.12:
        flags.append("cash_tax_mismatch")
        warnings.append("Cash tax paid is materially below reported profit-before-tax.")
    ...
    if cwip_to_gross is not None and cwip_to_gross > 0.40:
        flags.append("severe_cwip_bloat")
        warnings.append("Capital work-in-progress is too large relative to the gross block.")
    if debt_to_cfo is not None and debt_to_cfo > 6.0:
        flags.append("unsustainable_debt_to_cfo")
        hard_fail = True
    if fcf is not None and fcf < 0 and abs(dividends_paid or 0) > 0 and (total_debt or 0) > 0:
        flags.append("debt_funded_dividend_extraction")
        hard_fail = True
    ...
    return {
        ...
        "warnings": warnings,
        "hard_fail": hard_fail,
        "audit_status": "FAIL" if hard_fail else "PASS",
        "metrics": {
            ...
            "cash_tax_rate": None if cash_tax_rate is None else round(cash_tax_rate, 3),
            "debt_to_cfo": None if debt_to_cfo is None else round(debt_to_cfo, 2),
            "cwip_to_gross": None if cwip_to_gross is None else round(cwip_to_gross, 3),
            ...
            "free_cash_flow": None if fcf is None else round(fcf, 2),
        },
    }


def business_quality_score(fundamentals: dict[str, Any]) -> dict[str, Any]:
    normalized = apply_sector_median_fallbacks(fundamentals)
    medians = normalized.get("sector_medians") or {}
    revenue_abs = score_between(normalized.get("sales_cagr"), 0, 22)
    profit_abs = score_between(normalized.get("profit_cagr"), 0, 28)
    roce_abs = score_between(normalized.get("roce"), 8, 28)
    roe_abs = score_between(normalized.get("roe"), 8, 25)
    debt_abs = score_inverse(normalized.get("debt_equity"), 0.0, 1.8)
    revenue_score = revenue_abs * 0.65 + sector_relative_score(to_float(normalized.get("sales_cagr")), medians.get("sales_cagr"), True) * 0.35
    profit_score = profit_abs * 0.65 + sector_relative_score(to_float(normalized.get("profit_cagr")), medians.get("profit_cagr"), True) * 0.35
    roce_score = roce_abs * 0.6 + sector_relative_score(to_float(normalized.get("roce")), medians.get("roce"), True) * 0.4
    roe_score = roe_abs * 0.6 + sector_relative_score(to_float(normalized.get("roe")), medians.get("roe"), True) * 0.4
    debt_score = debt_abs * 0.7 + sector_relative_score(to_float(normalized.get("debt_equity")), medians.get("debt_equity"), False) * 0.3
    cfo_score = score_between(normalized.get("cfo_pat"), 0.45, 1.05)
    fcf_map = {"positive": 100, "improving": 82, "volatile": 55, "negative": 20}
    fcf_score = fcf_map.get(str(normalized.get("fcf_trend", "")).lower(), 45)
    promoter_trend = str(normalized.get("promoter_holding_trend", "stable")).lower()
    promoter_score = {"rising": 100, "stable": 85, "flat": 78, "falling": 35}.get(promoter_trend, 65)
    promoter_score = min(promoter_score, promoter_pledge_score(float(normalized.get("pledge_percent", 0) or 0)))
    if normalized.get("dilution_flag"):
        promoter_score -= 20
    margin_score = score_between(normalized.get("margin_trend_bps"), -250, 350)
    capital_efficiency = capital_efficiency_score(normalized, roce_score, roe_score)
    valuation_score = valuation_sanity_score(normalized, medians)
    forensic = forensic_earnings_quality(normalized)
    ...
    return {
        ...
        "data_quality": business_data_quality(normalized),
...
def nearest_tick(value: float, tick: float = 0.05, direction: str = "up") -> float:
    ...
    return round(scaled * tick, 2)


def classical_pivot_levels(bar: dict[str, Any]) -> dict[str, float]:
    high = float(bar.get("high") or bar.get("close") or 0)
    low = float(bar.get("low") or bar.get("close") or 0)
    close = float(bar.get("close") or 0)
    pivot = (high + low + close) / 3 if high and low and close else close
    spread = high - low
    return {
        "pivot": round(pivot, 2),
        "r1": round(2 * pivot - low, 2),
        "s1": round(2 * pivot - high, 2),
        "r2": round(pivot + spread, 2),
        "s2": round(pivot - spread, 2),
    }


def fibonacci_levels(low_value: float, high_value: float) -> dict[str, float]:
    if high_value <= low_value:
        return {}
    spread = high_value - low_value
    return {
        "fib_382": round(high_value - spread * 0.382, 2),
        "fib_500": round(high_value - spread * 0.500, 2),
        "fib_618": round(high_value - spread * 0.618, 2),
        "fib_786": round(high_value - spread * 0.786, 2),
    }
...
def technical_strength_score(
    ...
    avg_vol_50 = mean(volumes[-51:-1]) if len(volumes) >= 51 else avg_vol
    volume_ratio = volumes[-1] / avg_vol if avg_vol else 1.0
    volume_consistency_ratio = volumes[-1] / avg_vol_50 if avg_vol_50 else 1.0
    ...
    volume_consistency_pass = volume_consistency_ratio >= 0.8
    delivery_pct = to_float((event_context or {}).get("delivery_pct"))
    delivery_pass = delivery_pct is None or delivery_pct >= 40.0
    ...
    checks = {
        ...
        "volume_consistency": volume_consistency_pass,
        "delivery_accumulation": delivery_pass,
    ...
    weights = {
        ...
        "atr_breakout": 8,
        "volume_confirmed": 8,
        "volume_consistency": 7,
        "delivery_accumulation": 8,
        "vpt_accumulation": 8,
        "obv_accumulation": 4,
    ...
    if (breakout_fast or breakout_slow) and volume_ratio < 1.3:
        ...
        score -= 8
    if (breakout_fast or breakout_slow or atr_breakout) and not volume_consistency_pass:
        fake_breakout_flags.append("Current volume is below 80 percent of 50-day average; liquidity is not consistent")
        score -= 8
    if (breakout_fast or breakout_slow or atr_breakout) and delivery_pct is not None and not delivery_pass:
        fake_breakout_flags.append("Delivery percentage is below 40 percent; move may be intraday speculation")
        score -= 8
    ...
    trigger_price = nearest_tick(breakout_level + 0.05, 0.05, "up")
    recent_low_lookback = min(10, len(working_bars))
    recent_low = min(float(bar["low"]) for bar in working_bars[-recent_low_lookback:])
    support_20d = min(float(bar["low"]) for bar in working_bars[-min(profile["breakout_fast"], len(working_bars)) :])
    support_55d = min(float(bar["low"]) for bar in working_bars[-min(profile["breakout_slow"], len(working_bars)) :])
    pivot_levels = classical_pivot_levels(last_bar)
    high_52w_window = working_bars[-min(252, len(working_bars)) :]
    high_52w = max(float(bar["high"]) for bar in high_52w_window)
    low_52w = min(float(bar["low"]) for bar in high_52w_window)
    fib_levels = fibonacci_levels(low_52w, high_52w)
    setup_width_pct = pct_distance(breakout_level, support_20d) if support_20d else 0.0
    stalking = current < trigger_price and setup_width_pct <= 8 and pct_distance(trigger_price, current) <= 3.5
    ...
    aggressive_low = max(current, trigger_price)
    ...
    risk_per_share = max(trigger_price - max(stop, 0), 0)
    ...
    target_1 = round(trigger_price + risk_per_share * 2, 2) if risk_per_share > 0 else None
    target_2 = round(trigger_price + risk_per_share * 3, 2) if risk_per_share > 0 else None
    setup_state = (
        "Triggered"
    ...
    support_levels = {
        "trend": round(fast_ma, 2),
        "positional": round(medium_ma, 2),
        "support_20d": round(support_20d, 2),
        "support_55d": round(support_55d, 2),
        "pivot_s1": pivot_levels["s1"],
    }
    resistance_levels = {
        "trigger": round(trigger_price, 2),
        "resistance_20d": round(fast_breakout, 2),
        "resistance_55d": round(slow_breakout, 2),
        "pivot_r1": pivot_levels["r1"],
        "pivot_r2": pivot_levels["r2"],
    }
    ...
    return {
        ...
        "indicators": {
            ...
            "avg_volume_50": round(avg_vol_50, 2),
            "volume_consistency_ratio": round(volume_consistency_ratio, 2),
            ...
            "vcp_pattern": vcp,
            "setup_width_pct": round(setup_width_pct, 2),
        ...
        "entry": {
            "breakout_level": round(breakout_level, 2),
            "buy_stop_trigger": round(trigger_price, 2),
            "buy_stop_limit": round(trigger_price, 2),
            ...
            "target_1": target_1,
            "target_2": target_2,
            "risk_reward_target_1": round((target_1 - trigger_price) / risk_per_share, 2) if target_1 and risk_per_share > 0 else None,
            "risk_reward_target_2": round((target_2 - trigger_price) / risk_per_share, 2) if target_2 and risk_per_share > 0 else None,
            "setup_state": setup_state,
            "support_levels": support_levels,
            "resistance_levels": resistance_levels,
            "pivot_levels": pivot_levels,
            ...
            "position_sizing": {
                ...
                "entry_reference": round(trigger_price, 2),
...
def market_support_score(market: dict[str, Any]) -> dict[str, Any]:
    ...
    smallcap_rs = market.get("smallcap_relative_strength") or {}
    if smallcap_rs.get("smallcap_entries_restricted") and master_switch.get("regime_status") == "GREEN":
        master_switch = {
            **master_switch,
            "position_multiplier": min(float(master_switch.get("position_multiplier", 1.0) or 1.0), 0.5),
            "smallcap_entries_restricted": True,
            "smallcap_rs": smallcap_rs,
            "description": f"{master_switch.get('description', '')} Smallcap RS is below its 20-day average, so mid/smallcap entries are restricted to half size.",
        }
    else:
        master_switch = {
            **master_switch,
            "smallcap_entries_restricted": bool(smallcap_rs.get("smallcap_entries_restricted")),
            "smallcap_rs": smallcap_rs,
        }
    ...
    smallcap_rs_penalty = 5 if master_switch.get("smallcap_entries_restricted") else 0
    adjusted_total = int(clamp(total - stale_penalty - smallcap_rs_penalty))
    if smallcap_rs_penalty:
        breakdown["smallcap_relative_strength_down"] = {"score": 35, "weight": 0, "contribution": -smallcap_rs_penalty}
    ...
    return {
        ...
        "smallcap_relative_strength": smallcap_rs,
        "vix": vix,
...
def risk_penalty(
    ...
    for flag in forensic.get("flags", []):
        ...
        elif flag == "cash_tax_mismatch":
            penalties["cash_tax_mismatch"] = 8
        elif flag == "severe_cwip_bloat":
            penalties["severe_cwip_bloat"] = 10
        elif flag == "unsustainable_debt_to_cfo":
            penalties["unsustainable_debt_to_cfo"] = 18
        elif flag == "debt_funded_dividend_extraction":
            penalties["debt_funded_dividend_extraction"] = 20
...
def compute_verdict(
    ...
    risk_score: int,
    market_result: dict[str, Any],
    data_complete_enough: bool,
    failed_gates: list[str],
) -> str:
    if not data_complete_enough:
        return "Insufficient data - sync Yahoo/Supabase history and fundamentals before trusting this score"
    master_switch = market_result.get("master_switch") or {}
    if not bool(master_switch.get("can_buy", market_result.get("regime") != "Risk-off")):
        return "Market regime is RED - no new buys; manage existing stops only"
    if "rubber_band_overextension_limit" in failed_gates:
        return "Avoid - price is overextended above the 200 DMA"
    if "forensic_earnings_quality_fail" in failed_gates:
        return "Avoid - forensic earnings-quality gate failed"
    if "portfolio_risk_matrix_failed" in failed_gates or any("portfolio" in gate for gate in failed_gates):
        return "Blocked - portfolio concentration or correlation cap is active"
    if risk_score >= 35:
        return "Avoid - risk penalties are too high"
    if candidate and weekly >= 75 and monthly >= 75:
        return "Strong candidate - consider only at the planned entry with defined stop"
    if candidate:
        return "Candidate - monitor the trigger and position size from the entry plan"
    if weekly >= 60 or monthly >= 60:
        return "Near candidate - watch for missing gates to improve"
    return "Not a candidate - keep on screen only, no action"
...
def build_action_plan(
    ...
    market_result: dict[str, Any],
    price_data_quality: dict[str, Any],
    failed_gates: list[str],
    risk_score: int,
) -> dict[str, Any]:
    ...
    current_price = float(indicators.get("close", 0) or 0)
    trigger_price = to_float(entry.get("buy_stop_trigger") or entry.get("breakout_level"))
    stop = to_float(entry.get("stop"))
    target_1 = to_float(entry.get("target_1"))
    target_2 = to_float(entry.get("target_2"))
    setup_state = entry.get("setup_state") or indicators.get("setup_state") or "Screened"
    market_open_for_buys = bool((market_result.get("master_switch") or {}).get("can_buy", market_result.get("regime") != "Risk-off"))
    trigger_distance_pct = None
    if trigger_price and current_price:
        trigger_distance_pct = round((trigger_price - current_price) / max(current_price, 0.01) * 100, 2)
    triggered = bool(trigger_price and current_price >= trigger_price)
    near_trigger = bool(trigger_distance_pct is not None and 0 <= trigger_distance_pct <= 3.0)
    if not bool(price_data_quality.get("pass")):
        action = "WAIT_DATA"
        confidence = "low"
        summary = "History/fundamental completeness is not strong enough for a trusted decision."
    elif not market_open_for_buys:
        action = "AVOID"
        confidence = "high"
        summary = "Market regime is risk-off, so the engine blocks new swing buys."
    elif candidate and triggered:
        action = "BUY"
        confidence = "high" if risk_score <= 20 else "medium"
        summary = "Breakout is active. Buy only inside the planned risk box with the stop already defined."
    elif candidate and near_trigger:
        action = "STALK"
        confidence = "medium"
        summary = "Setup is close to the breakout trigger. Wait for the price/volume confirmation rather than chasing."
    elif candidate:
        action = "WATCH"
        confidence = "medium"
        summary = "The setup passed the engine, but price has not reached the ideal trigger yet."
    else:
        action = "AVOID"
        confidence = "medium" if risk_score < 35 else "high"
        summary = "The setup is not fully aligned yet. Treat it as screen-only research."
    return {
        "action": action,
        ...
        "aggressive_zone": entry.get("aggressive"),
        "entry_zone": entry.get("aggressive") or entry.get("pullback"),
        "pullback_zone": entry.get("pullback"),
        ...
        "risk_reward_target_1": entry.get("risk_reward_target_1"),
        "risk_reward_target_2": entry.get("risk_reward_target_2"),
        "support_levels": entry.get("support_levels") or indicators.get("support_levels") or {},
        "resistance_levels": entry.get("resistance_levels") or indicators.get("resistance_levels") or {},
        "pivot_levels": entry.get("pivot_levels") or indicators.get("pivot_levels") or {},
        "fib_levels": entry.get("fib_levels") or indicators.get("fib_levels") or {},
        "risk_capital": sizing.get("risk_capital"),
        "suggested_quantity": sizing.get("suggested_quantity"),
        "approx_position_value": sizing.get("approx_position_value"),
        "account_size": sizing.get("account_size"),
        "checklist_before_action": [
            "Wait for the trigger with price and volume confirmation.",
            "Confirm market regime is not risk-off before acting.",
            "Respect the stop and position size already defined by the engine.",
            "Review fresh events and data-quality warnings before taking exposure.",
        ],
        "failed_gates": failed_gates[:6],
    }


def build_premium_tags(
    ...
    vcp = indicators.get("vcp_pattern") or {}
    if vcp.get("score", 0) >= 75:
        tags.append({
            "label": "Volatility Contraction",
            "type": "bullish",
            "description": f"Base is tightening with VCP score {vcp.get('score')}/100 and setup state {indicators.get('setup_state', 'active')}.",
        })
    if indicators.get("volume_dryup_ratio") is not None and float(indicators.get("volume_dryup_ratio", 1.0)) <= 0.65:
        tags.append({
            "label": "Extreme Volume Dry-up",
            "type": "bullish",
            "description": f"Recent volume has dried to {indicators.get('volume_dryup_ratio')}x of the broader base average, which suggests supply exhaustion.",
        })
    if indicators.get("vpt", {}).get("score", 0) >= 65 or indicators.get("obv", {}).get("score", 0) >= 60:
        tags.append({
            "label": "Institutional Accumulation",
            "type": "bullish",
            "description": f"VPT {indicators.get('vpt', {}).get('score', 'NA')} and OBV {indicators.get('obv', {}).get('score', 'NA')} point to accumulation.",
        })
    pledge = to_float(company.get("fundamentals", {}).get("pledge_percent"), 0) or 0
    if pledge == 0:
        tags.append({
            "label": "Zero Promoter Pledge",
            "type": "safe",
            "description": "No promoter pledge is currently flagged, reducing governance overhang.",
        })
    forensic = business.get("forensic_quality", {})
    if forensic.get("hard_fail"):
        tags.append({
            "label": "Forensic Veto",
            "type": "bearish",
            "description": "Audit-grade balance-sheet or cash-flow red flags are active, so the engine blocks conviction.",
        })
    elif forensic.get("score", 0) >= 70:
        tags.append({
            "label": "Forensic Health",
            "type": "safe",
            "description": f"Audit quality scores {forensic.get('score')}/100 with no hard-fail flags.",
        })
    if event_result.get("score", 0) >= 65:
        tags.append({
            "label": "Fresh Catalyst",
            "type": "bullish",
            "description": "The current event layer is supportive enough to matter for a swing setup.",
        })
    if risk.get("score", 0) >= 30:
        tags.append({
            "label": "Risk Load Elevated",
            "type": "bearish",
            "description": f"Rule-based penalties are high at {risk.get('score')}/100, so setup quality is capped.",
        })
    return tags[:6]
...
def build_research_summary(
    ...
    time_horizon = "Long-term research" if monthly > weekly + 6 else "Short-term swing"
    setup_state = technical.get("indicators", {}).get("setup_state") or action_plan.get("setup_state") or "Screened"
    trigger = action_plan.get("trigger_price")
    primary_risk = next(iter(risk.get("breakdown", {}).keys()), None) or "Market/data risk needs review."
    if not data_quality_gate.get("pass"):
        why_not_actionable = data_quality_gate.get("warning") or "WAIT_DATA: ranking blocked until data quality improves."
    elif not candidate:
        why_not_actionable = action_plan.get("reason_summary") or "The setup is still research-only."
    else:
    ...
    return {
        ...
        "setup_state": setup_state,
        "why_interesting": (
            f"{symbol} has a {weekly}/100 weekly setup score and a {monthly}/100 longer-horizon score, "
            f"so it belongs on the {time_horizon.lower()} desk."
        ),
        "why_not_actionable": why_not_actionable,
        "main_trigger": f"Watch {round(trigger, 2)} as the main confirmation level." if trigger else "No valid trigger until price/data quality improve.",
        "main_risk": str(primary_risk).replace("_", " "),
        "orientation": "long_term" if monthly > weekly + 6 else "short_term",
    }
...
def build_short_term_view(
    ...
    return {
        ...
        "setup_state": action_plan.get("setup_state") or indicators.get("setup_state"),
        "volume_confirmation": event_result.get("score", 0) >= 55 or bool(indicators.get("volume_ratio", 0) and float(indicators.get("volume_ratio", 0)) >= 1.2),
        "relative_strength_vs_nifty": {
            "state": relative_strength.get("state"),
            "pct": relative_strength.get("pct"),
        },
        "trigger": action_plan.get("trigger_price"),
        "pullback_zone": action_plan.get("pullback_zone"),
        "stop": action_plan.get("stop"),
        "target_1": action_plan.get("target_1"),
        "target_2": action_plan.get("target_2"),
        "risk_reward_target_1": action_plan.get("risk_reward_target_1"),
        "risk_reward_target_2": action_plan.get("risk_reward_target_2"),
        "market_regime": market_result.get("regime"),
    }


def build_breakdown_summary(breakdown: dict[str, Any], label: str) -> str:
    if not breakdown:
        return f"No detailed {label} breakdown is available yet."
    ranked = sorted(
        ((str(key).replace("_", " "), to_float(value, 0) or 0) for key, value in breakdown.items()),
        key=lambda item: item[1],
        reverse=True,
    )
    top = ranked[:3]
    return ", ".join(f"{name} {round(score, 1)}" for name, score in top)
...
def build_long_term_view(
    ...
    return {
        "score": monthly,
        "business_quality_score": business.get("score"),
        "sector_tailwind_score": tailwind.get("score"),
        "valuation_score": business.get("valuation_score"),
        "metrics": {
            "roce": fundamentals.get("roce"),
            "roe": fundamentals.get("roe"),
            "sales_cagr": fundamentals.get("sales_cagr"),
            "profit_cagr": fundamentals.get("profit_cagr"),
            "debt_equity": fundamentals.get("debt_equity"),
            "pe": fundamentals.get("pe"),
        },
        "business_quality_explanation": build_breakdown_summary(business.get("breakdown", {}), "business quality"),
        "sector_tailwind_explanation": build_breakdown_summary(tailwind.get("breakdown", {}), "sector tailwind"),
        "watchlist_only": not data_quality_gate.get("pass"),
    }
...
def build_risk_dashboard(
    ...
    fake_flags = technical.get("fake_breakout_flags", [])
    return {
        "business_risk": "Elevated" if to_float(fundamentals.get("debt_equity"), 0) > 1 else "Normal",
        "valuation_risk": "Elevated" if to_float(fundamentals.get("pe"), 0) > 40 else "Normal",
        "technical_risk": ", ".join(fake_flags[:3]) if fake_flags else "No major breakout trap flag.",
        "data_risk": " | ".join((price_data_quality.get("warnings") or [])[:3]) or "No major price-history warning.",
        "market_risk": market_result.get("regime", "Unknown"),
        "event_risk": "Elevated" if event_result.get("negative_governance") else "Normal",
        "liquidity_risk": next((key for key in risk.get("breakdown", {}).keys() if "liquidity" in key.lower()), "No major liquidity flag."),
        "risk_flags": list(dict.fromkeys(list(risk.get("breakdown", {}).keys()) + fake_flags)),
    }


def build_pro_research_sections(
    company: dict[str, Any],
    business: dict[str, Any],
    tailwind: dict[str, Any],
    event_result: dict[str, Any],
    ...
    bull_case = [
        f"Business quality scores {business.get('score')}/100 and sector tailwind scores {tailwind.get('score')}/100.",
        f"Technical structure is {technical.get('score')}/100 with setup state {action_plan.get('setup_state') or technical.get('indicators', {}).get('setup_state', 'Screened')}.",
    ]
    bear_case = [
        data_quality_gate.get("warning") or "Data quality must remain clean for the thesis to stay trusted.",
        f"Event layer score is {event_result.get('score')}/100, so weak follow-through can quickly cool the setup.",
    ]
    neutral_case = [
        f"{company.get('symbol')} stays research-only until the trigger and market regime line up cleanly.",
    ]
    return {
        "bull_case": bull_case,
        "bear_case": bear_case,
        "neutral_case": neutral_case,
        "checklist_before_action": action_plan.get("checklist_before_action", []),
    }
...
def build_data_quality_explanation(
    ...
    return {
        "pass": data_quality_gate.get("pass"),
        "message": data_quality_gate.get("warning") or ("Data quality gate passed." if data_quality_gate.get("pass") else "Data quality gate is blocked."),
        "missing_fields": data_quality_gate.get("missing_fields", []),
        "imputed_fields": data_quality_gate.get("imputed_fields", []),
        "bars_available": len(bars),
        "corporate_actions_applied": data_quality_gate.get("corporate_actions_applied"),
        "recent_source": connector_source,
        "latest_bar_date": latest_bar.get("datetime"),
        "real_vs_fallback": "Real history" if data_quality_gate.get("pass") else "Fallback/imputed inputs still present",
        "price_warnings": price_data_quality.get("warnings", []),
    }
...
def final_decision(
    ...
    fundamentals = apply_sector_median_fallbacks(
        {**company["fundamentals"], "sector": company.get("sector"), "industry": company.get("industry", "")}
    )
    ...
    technical = technical_strength_score(
        ...
        event_context={
            **event_volume_context(event_result.get("events", [])),
            ...
            "regime_position_multiplier": regime_position_multiplier,
            "delivery_pct": company.get("delivery_pct"),
        },
    )
    ...
    verdict = compute_verdict(candidate, weekly, monthly, risk["score"], market_result, data_complete_enough, failed_gates)
    action_plan = build_action_plan(candidate, verdict, {**technical, "entry": entry}, market_result, price_data_quality, failed_gates, risk["score"])
    premium_tags = build_premium_tags(company, {**technical, "entry": entry}, business, event_result, risk)
    gate_warning_parts: list[str] = []
    if data_quality.get("warning"):
        gate_warning_parts.append(str(data_quality.get("warning")))
    if not data_complete_enough:
        gate_warning_parts.append("Blocked from candidate status because fundamentals/price data quality did not pass")
    data_quality_gate = {
        ...
        "imputed_fields": data_quality.get("imputed_fields", []),
        "price_data_quality": price_data_quality,
        "corporate_actions_applied": price_data_quality.get("corporate_actions_applied"),
        "warning": " | ".join(dict.fromkeys(gate_warning_parts)) if gate_warning_parts else None,
    }
    ...
    short_term_view = build_short_term_view(weekly, technical, event_result, action_plan, market_result)
    long_term_view = build_long_term_view(monthly, business, tailwind, fundamentals, data_quality_gate)
    pro_research_sections = build_pro_research_sections(
        company,
        business,
        tailwind,
        event_result,
    ...
    risk_dashboard = build_risk_dashboard(risk, technical, price_data_quality, fundamentals, market_result, event_result)
    data_quality_explanation = build_data_quality_explanation(
        data_quality_gate,
        price_data_quality,
        "stored_scoring_bars",
        bars,
    ...
    return {
        ...
        "engine_scores": {
            ...
            "weekly_score": weekly,
            "monthly_score": monthly,
            "technical_strength": technical["score"],
            "fundamental_forensic": business.get("forensic_quality", {}).get("score"),
        ...
        "forensic_audit": {
            "status": business.get("forensic_quality", {}).get("audit_status", "PASS"),
            "score": business.get("forensic_quality", {}).get("score"),
            "hard_fails": [flag for flag in business.get("forensic_quality", {}).get("flags", []) if business.get("forensic_quality", {}).get("hard_fail")],
            "warnings": business.get("forensic_quality", {}).get("warnings", []),
            "metrics": business.get("forensic_quality", {}).get("metrics", {}),
        },
        ...
        "pro_research_sections": pro_research_sections,
...
def build_thesis(
    ...
    lines = [
        ...
        (
            f"Technical score is {technical['score']}/100 with buy stop "
            f"{technical['entry'].get('buy_stop_trigger', technical['entry']['breakout_level'])}, "
            f"stop {technical['entry']['stop']}, target 1 {technical['entry'].get('target_1')}, "
            f"and target 2 {technical['entry'].get('target_2')}."
        ),
    ]
    if risk["score"] > 0:
