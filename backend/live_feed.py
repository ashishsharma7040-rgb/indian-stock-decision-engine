        ...
        try:
            ...
            for host, websocket in self._endpoint_candidates():
                ...
                with self._lock:
                    ...
                    self._last_error = None
                logger.info("Shoonya attempting endpoint host=%s websocket=%s", host, websocket)
                try:
                    try:
                        ...
                        raise RuntimeError(
                            f"Shoonya returned an empty/non-JSON login response from {host}. Common causes: wrong password/API key, "
                    ...
                    logger.warning("Shoonya endpoint attempt failed for host=%s: %s", host, endpoint_exc)
