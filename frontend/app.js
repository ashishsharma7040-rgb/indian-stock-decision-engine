    ...
    <div class="settings-grid">
      ...
      ${renderSettingsSection(
        ...
        `
          ...
          ${renderSettingsChecklistRow("TOTP / OTP available", Boolean(live.credential_flags?.totp_secret || live.credential_flags?.twofa), live.totp_source || "No runtime OTP source seen yet.")}
          ...
          ${renderSettingsChecklistRow("Login successful", live.last_login_response_stat === "Ok" || live.status === "logged_in" || live.feed_open, live.last_login_error || live.last_error || "No successful login recorded yet.")}
          ...
          <section style="margin-top:12px">
            ...
            <div class="status-line">Use this only when you do not have a working TOTP secret on Render. The value is sent to backend only and is never stored in the frontend.</div>
        ...
        `,
        `<div class="card-actions"><button class="btn blue" data-refresh-live-debug="1">Refresh live debug</button><button class="btn" data-restart-live="1">Start / retry live feed</button><button class="btn" id="settingsSubmitShoonyaOtpBtn">Submit OTP</button></div>`
      )}
      ${renderSettingsSection(
        "Database / Supabase",
        "Persistence",
...
document.addEventListener("click", event => {
  ...
  if (event.target.closest("[data-refresh-live-debug]")) refreshLiveDebugStatus(true);
  if (event.target.closest("[data-refresh-db-status]")) refreshDatabaseStatus(true);
  ...
  if (event.target.closest("[data-restart-live]")) {
    getJson("/api/live/restart", {method: "POST"})
      ...
      .catch(error => setNotice(`Shoonya restart failed: ${error.message}`, "red"));
    return;
  }
