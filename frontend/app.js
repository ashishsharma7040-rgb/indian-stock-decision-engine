...
async function refreshDatabaseStatus(showNotice = false) {
  ...
  if (showNotice) {
    ...
    const msg = status.enabled
      ...
      : "Supabase is not connected. Add DATABASE_URL in Render.";
    setNotice(msg, status.enabled && !counts.bhavcopy_stale ? "blue" : "amber");
  }
  renderMarketStrip();
  return status;
}
...
function renderMarketStrip() {
  ...
  const chips = [
    ...
    ["Supabase", `<span class="regime-pill ${dbTone}">${escapeHtml(dbLabel)}</span>`],
...
function normalizeFullScanPayload(payload) {
  ...
  const realRows = stocks.filter(stock => {
    return !isFullScan || ["real_nse_scored", "nse_eod_proxy_scored", "nse_eod_batch_scored", "nse_dynamic_real", "nse_dynamic_focus"].includes(stock.data_mode || "");
  });
...
async function runFullNseScan() {
  ...
  const applyFullScanPayload = (payload) => {
    ...
    setScanUi(`Scan complete. Universe ${countText(scanMeta.universe_size)}, passed ${countText(scanMeta.passed_liquidity)}, scored ${countText(scanMeta.total_scored)}${proxyText}, ranked ${countText(scanMeta.ranked_candidates)}.${basisMore}`);
  };
  ...
  const pollStatus = async () => {
    ...
    if (status.status === "complete") {
      ...
      idlePolls += 1;
      if (idlePolls >= 2) {
        setNotice("Backend did not keep the scan worker alive. Restarting the scan once; if this repeats, redeploy the latest backend zip.", "amber");
        const retry = await getJson("/api/admin/run-full-nse-scan", {
          method: "POST",
          headers: {"Content-Type": "application/json"},
          body: JSON.stringify({scan_limit: "all", force: false})
        });
        scanMeta = {...(scanMeta || {}), ...(retry.scan_meta || retry.scan_status?.scan_meta || {})};
        idlePolls = 0;
      }
    } else {
      idlePolls = 0;
