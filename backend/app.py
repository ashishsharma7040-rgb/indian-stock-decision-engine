...
def merge_database_fundamentals(symbol: str, company: dict[str, Any], connector_notes: list[str]) -> None:
    if not DATABASE_ENABLED or latest_fundamentals_from_db is None:
        return
    try:
        payload = _run_async_sync(latest_fundamentals_from_db([symbol]))
...
def load_database_research_bars(
    ...
    if not DATABASE_ENABLED or historical_bars_from_db is None:
        return [], False
    try:
        payload = _run_async_sync(historical_bars_from_db([symbol], days=260, min_bars=min_bars))
        bars = (payload.get("bars_by_symbol") or {}).get(symbol.upper()) or []
        if len(bars) >= min_bars:
...
def dynamic_universe_stock_detail(symbol: str) -> dict[str, Any]:
    ...
    bars, corporate_actions_applied = load_database_research_bars(symbol, row, connector_notes, min_bars=210)
...
def score_full_scan_row_fast(
    ...
    try:
        ...
        company["tailwind_factors"] = [
            ...
            "If Yahoo enrichment exists in Supabase, the scan uses that stored history before falling back to proxy bars.",
        ]
        data_mode = "nse_eod_batch_scored"
        price_source = "NSE bhavcopy EOD"
        notes = ["Batch full-scan mode: no per-symbol network calls were made."]
        if db_history and len(db_history) >= 200:
            bars = merge_scan_row_into_history(db_history, row)
            data_mode = "supabase_yahoo_history_scored"
            price_source = "Supabase Yahoo-enriched history with latest bhavcopy overlay"
            notes.append(f"Used {len(bars)} stored historical bars from Supabase enrichment.")
        else:
            bars = build_scan_proxy_bars_from_row(row, days=240)
            notes.append("No sufficient stored history; scored from deterministic NSE EOD proxy bars.")
            if len(bars) < 200:
                return None
        benchmark_bars = market_result.get("_benchmark_bars") or DATASET["benchmark_bars"]
...
def yahoo_enrich_status_payload() -> dict[str, Any]:
    ...
    payload["worker_alive"] = bool(YAHOO_ENRICH_TASK and not YAHOO_ENRICH_TASK.done())
    return payload
...
def build_full_nse_scan_payload(scan_limit: Any = "all", force: bool = False) -> dict[str, Any]:
    ...
    """
    max_workers = max(1, min(int(os.getenv("SCAN_WORKERS", "4")), 8))
    ...
    try:
        ...
        symbols_to_score = [str(row.get("symbol", "")).upper() for row in rows_to_score if row.get("symbol")]
        history_by_symbol: dict[str, list[dict[str, Any]]] = {}
        fundamentals_by_symbol: dict[str, dict[str, Any]] = {}
        history_meta: dict[str, Any] = {"enabled": False, "status": "not_used"}
        fundamentals_meta: dict[str, Any] = {"enabled": False, "status": "not_used"}
        if DATABASE_ENABLED and historical_bars_from_db is not None:
            set_full_scan_status("running", 13, "Loading enriched Yahoo history from Supabase for scan scoring.")
            try:
                history_payload = _run_async_sync(historical_bars_from_db(symbols_to_score, days=260, min_bars=200))
                ...
                history_meta = {"enabled": True, "status": "error", "error": str(exc)}
        if DATABASE_ENABLED and latest_fundamentals_from_db is not None:
            try:
                fundamentals_payload = _run_async_sync(latest_fundamentals_from_db(symbols_to_score))
                fundamentals_by_symbol = dict(fundamentals_payload.get("fundamentals_by_symbol") or {})
                fundamentals_meta = {key: value for key, value in fundamentals_payload.items() if key != "fundamentals_by_symbol"}
            except Exception as exc:
                fundamentals_meta = {"enabled": True, "status": "error", "error": str(exc)}
        set_full_scan_status(
            "running",
            15,
        ...
        enriched_count = sum(1 for stock in summaries if stock.get("data_mode") == "supabase_yahoo_history_scored")
        allowed_full_scan_modes = {"real_nse_scored", "supabase_yahoo_history_scored", "nse_eod_proxy_scored", "nse_eod_batch_scored", "nse_dynamic_real", "nse_dynamic_focus"}
        strict_ranked = [
            stock
        ...
        def rank_key(stock: dict[str, Any], score_key: str) -> tuple[int, int, int]:
            data_bonus = 3 if stock.get("data_mode") in {"real_nse_scored", "supabase_yahoo_history_scored"} else 1
