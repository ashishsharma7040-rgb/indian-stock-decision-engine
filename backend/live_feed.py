...
class LiveFeedManager:
    ...
    def _run(self) -> None:
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    "Shoonya returned an empty/non-JSON login response. Common causes: wrong password/API key, "
    ...
    def subscribe(self, symbols: list[str]) -> dict[str, Any]:
        ...
        if api is None or not self._feed_open:
            with self._lock:
                self._unresolved_symbols.update(str(symbol or "").upper().strip() for symbol in symbols if str(symbol or "").strip())
    ...
    def _resolve_token(self, api: Any, symbol: str) -> str | None:
        try:
            ...
            logger.warning("Shoonya searchscrip failed for %s: %s", symbol, exc)
            return None
