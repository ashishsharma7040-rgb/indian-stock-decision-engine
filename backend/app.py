...
def refresh_symbol(symbol: str) -> dict[str, Any]:
    ...
    bars = []
    try:
        bars = fetch_yahoo_intraday(symbol)
        if len(bars) >= 60:
            connector_notes.append("Yahoo Finance 5-minute bars loaded without API key")
    except Exception as exc:  # pragma: no cover - network dependent
        connector_notes.append(f"Yahoo Finance intraday failed: {exc}")
    if len(bars) < 60:
        try:
            bars = fetch_yahoo_chart(symbol)
            if len(bars) >= 60:
                connector_notes.append("Yahoo Finance daily bars loaded without API key")
        except Exception as exc:  # pragma: no cover - network dependent
            connector_notes.append(f"Yahoo Finance daily fallback failed: {exc}")
            bars = []
    if len(bars) < 60:
